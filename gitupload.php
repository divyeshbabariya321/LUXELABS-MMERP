...
if (isset($_POST['action']) && $_POST['action'] == 'process') {
    ...
    if (isset($sanaticedLines) && ! empty($sanaticedLines)) {
        ...
        $conn_id = ftp_connect($server[$_POST['server']]['host'], 21) or exit('Cannot connect to host');

        // send access parameters
        ftp_login($conn_id, $server[$_POST['server']]['usr'], $server[$_POST['server']]['pwd']) or exit('Cannot login');
