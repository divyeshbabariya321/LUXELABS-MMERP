...
Sql.getFieldName = function ($tableResults, $thisField) {
  ...
  var leftActionSkip = leftActionExist
    ...
    : 0; // If this column was sorted, the text of the a element contains something
  ...
  if (fieldName === "") {
    var $heading = $tableResults
      ...
      .find("th")
      .eq(thisFieldIndex - leftActionSkip)
      .children("span"); // may contain column comment enclosed in a span - detach it temporarily to read the column name
...
AJAX.registerOnload("sql.js", function () {
  ...
  $(function () {
    if (codeMirrorEditor) {
      ...
      var useLocalStorageValue =
        ...
        typeof window.localStorage.autoSavedSqlSort !== "undefined"; // Save sql query with sort
      ...
      if (
        ...
        $('select[name="sql_query"] option[value="' + sortStoredQuery + '"]')
          .length !== 0
      ) {
  ...
  $(document).on("click", "#copyToClipBoard", function (event) {
    ...
    $(".table_results tbody tr").each(function () {
      $(this)
        ...
        .each(function () {
          ...
          var data =
            $(this).find("em").length !== 0 ? $(this).find("em")[0] : this;
    ...
    try {
      ...
      toastr["error"]("Sorry! Unable to copy", "error");
    } // eslint-disable-next-line compat/compat
  ...
  if (!$("#sqlqueryform").find("button").is("#togglequerybox")) {
    $('<button class="btn btn-secondary" id="togglequerybox"></button>')
      ...
      .appendTo("#sqlqueryform") // initially hidden because at this point, nothing else
        ...
        if (typeof data !== "undefined" && data.success === true) {
          ...
          if (typeof data.action_bookmark !== "undefined") {
            ...
            if ("2" === data.action_bookmark) {
              ...
              ).remove(); // if there are no bookmarked queries now (only the empty option),
          ...
          if (typeof data.action_bookmark === "undefined") {
            if (
              $('#sqlqueryform input[name="retain_query_box"]').is(
                ":checked"
              ) !== true
            ) {
...
Sql.changeClassForColumn = function ($thisTh, newClass, isAddClass) {
  ...
  var hasBigT = $thisTh
    ...
    .children()
    .first()
    .hasClass("column_action"); // .eq() is zero-based
