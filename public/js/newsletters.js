var page = {
  ...
  loadFirst: function () {
    var _z = {
      url: this.config.baseUrl + "/newsletters/records",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/newsletters/records",
      method: "get",
  ...
  deleteRecord: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/newsletters/" + ele.data("id") + "/delete",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  createRecord: function (response) {
    var createWebTemplate = $.templates("#form-create-newsletters");
  ...
  editRecord: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/newsletters/" + ele.data("id") + "/edit",
      method: "get",
    };
  ...
  editResult: function (response) {
    ...
    $("#sent_at").datetimepicker({ format: "YYYY-MM-DD HH:mm:00" });
  },
  submitPlatform: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/newsletters/store",
      method: "post",
  ...
  pureFunction: function (response) {
	...
	getMultiScore: function(ele) {
		var numberOfChecked = $('.check-product').filter(':checked').length;
		var totalCheckboxes = $('.check-product').length;
		var numberNotChecked = totalCheckboxes - numberOfChecked;
		if (totalCheckboxes != numberNotChecked) {
			var searchIDs = $(".check-product:checked").map(function(){
				return $(this).val();
			}).get();
			var _z = {
				url:
					this.config.baseUrl +
					"/newsletters/get-multi-translated-text-score",
					method: "post",
					data: {ids: searchIDs},
					beforeSend: function () {
						$("#loading-image").show();
...
$(document).ready(function () {
  $("body").delegate("#pr-start-time, #pr-end-time", "focusin", function () {
    $("#pr-start-time, #pr-end-time").datetimepicker({
...
$(document).on("click", ".show-more-image", function () {
