...
var page = {
  ...
  editRecord: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/" + ele.data("id") + "/edit",
      method: "get",
    };
  ...
  createDatabase: function (ele) {
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/" + database_user_id + "/get-database",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  assignPermission: function (ele) {
    ...
    var _z = {
      url:
        this.config.mainUrl + "/" + database_user_id + "/assign-database-table",
      method: "post",
      data: ele.closest("form").serialize(),
      beforeSend: function () {
        $("#loading-image").show();
      ...
      url:
        typeof href != "undefined"
          ...
          : this.config.baseUrl +
            "/chat-messages/user/" +
            ele.data("id") +
            "/loadMoreMessages?limit=1000",
      method: "get",
    };
    this.sendAjax(_z, "chatListHistoryResult");
  },
  chatListHistoryResult: function (response) {
    var communicationHistoryTemplate = $.templates(
      "#template-communication-history"
    );
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl +
            "/user-management/user/delete-team/" +
            ele.data("id"),
      method: "post",
      data: ele.closest("form").serialize(),
      beforeSend: function () {
  ...
  submitPermission: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/permission/" + ele.data("id"),
      method: "post",
      data: ele.closest("form").serialize(),
      beforeSend: function () {
        $("#loading-image").show();
  ...
  roleModalOpen: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/user-management/role/" + ele.data("id"),
      method: "get",
    };
  ...
  submitRole: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/role/" + ele.data("id"),
      method: "post",
      data: ele.closest("form").serialize(),
      beforeSend: function () {
        $("#loading-image").show();
  ...
  addPermission: function () {
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/add-permission",
      method: "post",
      data: {
        name: name,
        route: route,
  ...
  permissionAdded: function (response) {
    if (response.code == 200) {
      ...
      $("#myUL").append(
        '<li style="list-style-type: none;"><a><input type="checkbox" name="permissions[]" value="' +
  ...
  saveTime: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/user-avaibility/submit-time",
      method: "post",
      data: ele.closest("form").serialize(),
      beforeSend: function () {
        $("#loading-image").show();
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl +
            "/user-management/user-avaibility/" +
            ele.data("id"),
      method: "get",
    };
  ...
  updateAvailability: function (ele) {
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/user-avaibility/" + ele.data("id"),
      method: "post",
      data: {
        note: note,
        status: status,
  ...
  approveUser: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.mainUrl + "/approve-user/" + ele.data("id"),
      method: "post",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  userProfileHistoryListing: function (userid) {
    var _z = {
      url:
        this.config.baseUrl +
        "/user-management/user-generate-file-listing/" +
        userid,
      method: "get",
    };
  ...
  deletePemUser: function (ele) {
    var _z = {
      url:
        this.config.baseUrl +
        "/user-management/delete-pem-file/" +
        ele.data("id"),
      method: "post",
    };
  ...
  if (e.which == 13) {
    ...
    if (type == "TASK") {
      $.ajax({
        ...
        success: function () {
          toastr["success"](
            "Estimate Minutes updated successfully!",
            "Message"
          );
        },
      });
...
$(document).on("click", ".set-due-date", function (e) {
  ...
  if (date != "") {
    ...
    toastr["error"]("Please enter a date first", "error");
  }
});
...
function updateUserFlagForTaskPlan(ele) {
  let btn = jQuery(ele);
  ...
  if (
    confirm(
      btn.data("is_task_planned") == 1
        ? "Are you sure? Do you want unflagged this user for task plan?"
        : "Are you sure? Do you want flagged this user for task plan?"
    )
  ) {
    jQuery.ajax({
      headers: {
      ...
      url: urlUserManagementUpdateFlagForTaskPlan,
      type: "POST",
      data: {
        user_id: user_id,
        is_task_planned: is_task_planned,
      },
      dataType: "json",
      beforeSend: function () {
        jQuery("#loading-image").show();
