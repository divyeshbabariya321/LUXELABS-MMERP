var siteHelpers = {
  ...
  changeQuickComment: function (ele) {
    ele
      .closest(".customer-raw-line")
      .find(".quick-message-field")
      .val(ele.val());
  },
  leadsChart: function () {
    var params = {
      ...
      templateResult: function (customer) {
        ...
        if (customer.name) {
          return (
            "<p> <b>Id:</b> " +
            customer.id +
            (customer.name ? " <b>Name:</b> " + customer.name : "") +
      ...
      templateResult: function (product) {
        ...
        if (product.sku) {
          return (
            "<p> <b>Id:</b> " +
            product.id +
            (product.name ? " <b>Name:</b> " + product.name : "") +
            " <b>Sku:</b> " +
            product.sku +
            " </p>"
          );
  ...
  loadCustomers: function (ele) {
    ...
    if (first_customer == second_customer) {
      ...
      toastr["error"]("You selected the same customers", "error");
      return;
    }
    var params = {
      data: {
      ...
      doneAjax: function (response) {
        ...
        $("#first_customer_phone").val(
          response.first_customer.phone
            ? response.first_customer.phone.replace(/[\s+]/g, "")
            : ""
        );
        $("#first_customer_instahandler").val(
          response.first_customer.instahandler
        );
        ...
        $("#second_customer_phone").val(
          response.second_customer.phone
            ? response.second_customer.phone.replace(/[\s+]/g, "")
            : ""
        );
        $("#second_customer_instahandler").val(
          response.second_customer.instahandler
        );
  ...
  erpLeadsSendMessage: function () {
    ...
    if (
      $("#send_message").find(".ddl-select-product").val() == "" &&
      $("#send_message").find("#product_start_date").val() == ""
    ) {
    ...
    var params = {
      ...
      data: {
        ...
        product_start_date: $("#send_message")
          .find("#product_start_date")
          .val(),
  ...
  instructionStore: function (ele) {
    var customer_id = ele
      .closest("form")
      .find('input[name="customer_id"]')
      .val();
    var instruction = ele
      .closest("form")
      .find('input[name="instruction"]')
      .val();
    var category_id = ele
      .closest("form")
        ...
        if (response.code == 1) {
          ...
          if (response.data.length > 0) {
            $.each(response.data, function (k, v) {
              html +=
                '<button class="badge badge-default broadcast-list-rndr" data-customer-id="' +
                customerId +
                '" data-id="' +
                v.id +
                '">' +
  ...
  broadcastListCreateLead: function (ele) {
    var $this = ele;
    var checkedProducts = $("#broadcast-list").find(
      "input[name='selecte_products_lead[]']:checked"
    );
    ...
    var selectionLead = $("#broadcast-list")
      .find(".selection-broadcast-list")
      .first();
    $("#broadcast-list-approval")
      .find(".broadcast-list-approval-btn")
      .data("customer-id", selectionLead.data("customer-id"));
    $("#broadcast-list-approval").modal("show");
    $(".broadcast-list-approval-btn")
      .unbind()
      .on("click", function () {
        var $this = $(this);
        var params = {
  ...
  changeStatus: function (ele) {
    ...
    var params = {
      ...
      doneAjax: function (response) {
        if (
          ele.hasClass("order_status") &&
          status == "Product shiped to Client"
        ) {
          $("#tracking-wrapper-" + id).css({
            display: "block",
          });
  ...
  sendMessageMaltiCustomer: function (ele) {
    ...
    if (!ele.is(":disabled")) {
      ...
      data.append(
        "quick_sell_groups",
        form.find("#product-quick-sell-groups").val()
      );
      var params = {
        method: "post",
        data: data,
  ...
  storeReminder: function (ele) {
    ...
    var customerIdToRemind = reminderModal
      .find('input[name="customer_id"]')
      .val();
    ...
    var reminder_last_reply = reminderModal
      .find("#reminder_last_reply")
      .is(":checked")
      ? 1
      : 0;
    var params = {
      ...
      doneAjax: function (response) {
        $(".set-reminder[data-id='" + customerIdToRemind + "']").data(
          "frequency",
          frequency
        );
        $(".set-reminder[data-id='" + customerIdToRemind + "']").data(
  ...
  addErpLead: function (ele, thiss) {
    ...
    if (ele.find(".multi_brand").val() == "") {
      ...
      toastr["error"]("Please Select Brand", "error");
      return false;
    }
    ...
    var params = {
      ...
      doneAjax: function (response) {
        ...
        $("#add_lead").modal("hide");
        if (
          $("#add_lead").find('input[name="product_id"]').length > 0 &&
          $("#add_lead").find('input[name="product_id"]').val()
        ) {
          var dataSending = $("#add_lead")
  ...
  addNextAction: function (ele) {
    ...
    if (textBox.val() == "") {
      toastr["error"]("Please Enter New Next Action!!", "error");
      return false;
    }
    var params = {
      method: "post",
  ...
  deleteNextAction: function (ele) {
    ...
    if (nextAction.val() == "") {
      ...
      toastr["error"]("Please Select Next Action!!", "error");
      return false;
    }
  ...
  addOrderFrm: function (ele, thiss) {
    ...
    var params = {
      ...
      doneAjax: function (response) {
        ...
        if (
          $("#add_order").find('input[name="selected_product[]"]').length > 0 &&
