  ...
  MessageUtils.getMessages()
    ...
    .forEach((msg) => {
      ...
      if (msg.recvIsGroup) {
        ...
        chat.contact = contactList.find((contact) =>
          msg.sender !== user.id
...
let viewChatList = () => {
  ...
  chatList
    ...
    .forEach((elem, index) => {
      ...
      DOM.chatList.innerHTML +=
        `
		<div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom ${unreadClass}" id="user-list-${
          elem.msg.recvId
        }">
          ...
          elem.isGroup
            ? contactList.find((contact) => contact.id === elem.msg.sender)
                .number + ": "
            : ""
        }${
          elem.msg.sender === user.id
            ...
            : ""
					...
					${
            elem.msg.has_media == true
              ? '<i class="fas fa-file-image"></i> Media'
              : elem.msg.body
          }
				</div>
			</div>
			<div class="flex-grow-1 text-right">
				...
				<div data-i="${
          elem.msg.recvId
        }" class="badge badge-danger badge-pill small block-from-list" onClick="blockEvent('` +
        elem.msg.recvId +
        `');">Block</div>
				${
          elem.unread
...
let addMediaToMessageArray = (msg, append) => {
  let sendStatus = `<i class="${
    msg.status <= 5 ? "far" : "fas"
  } fa-check-circle"></i>`;
      ...
      `">
				    <div class="_1b8RS">
				        <div class="_3SaET xCzoD">
				            ...
				            <div>
				                <div class="_3mdDl ${
                          key == 3 && msg.media.length > 4 ? "_1cdQD" : ""
                        }" style="width: 165px; height: 168px;">
				                   <img data-src="${media.url}" data-type="${
        media.type
      }" src="${
        media.type == "pdf" ? "/images/pdf_icon.png" : media.url
      }" class="_18vxA" style="height: 100%;">
				                   <div class="_3TrQs"></div>
				                </div>
				            </div>
				            ${dropMenu != "" ? dropMenu : ""}
				            ${
                      key == 3 && msg.media.length > 4
                        ? '<span class="_1XSxP _1drsQ">+' +
                          (msg.media.length - 4) +
                          "</span></div>"
                        : ""
                    }
  ...
  MediaHtml += "</div>";
  let unApprovedcls =
    msg.approved != 1 && !msg.isSender ? "unapproved-msg" : "";
  var body =
    `<div data-cn="${msg.id}" class="align-self-${
      msg.isSender ? "end self" : "start"
    } p-1 my-1 mx-3 rounded bg-white shadow-sm message-item ${
      msg.isLast == true ? "last-block-message" : ""
    } ${unApprovedcls}">
				<div class="options dropdown" style="z-index:1200">
					...
					<div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; transform: translate3d(-122px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">
						<a data-case="delete" data-i="${
              msg.id
            }" class="dropdown-item dropdown-item-message" href="javascript:;">Delete</a>
						${
              msg.approved != 1 &&
              (user.is_hod_crm || user.is_admin) &&
              !msg.isSender
                ? '<a data-case="approve" data-i="' +
                  msg.id +
                  '" class="dropdown-item dropdown-item-message" href="javascript:;">Approve</a>'
                : ""
            } 
						${
              msg.approved
                ? '<a data-case="resend" data-i="' +
                  msg.id +
                  '" class="dropdown-item dropdown-item-message" href="javascript:;">Resend</a>'
                : ""
            }
					</div>
				</div>
...
let addMessageToMessageArea = (msg, append) => {
  ...
  if (msg.has_media == false || msg.body != "") {
    var body = `<div data-cn="${msg.id}" class="align-self-${
      msg.isSender ? "end self" : "start"
    } p-1 my-1 mx-3 rounded bg-white shadow-sm message-item ${
      msg.isLast == true ? "last-block-message" : ""
    } ${unApprovedcls}">
            ...
            msg.approved
              ...
              : ""
					...
					${
            msg.status == 0
              ? '<a data-case="mark_as_read" data-i="' +
                msg.id +
                '" class="dropdown-item dropdown-item-message" href="javascript:;">Mark as Read</a>'
              : ""
          }
					${
            msg.status == 0 || msg.status == 5
              ? '<a data-case="mark_as_replied" data-i="' +
                msg.id +
                '" class="dropdown-item dropdown-item-message" href="javascript:;">Mark as Replied</a>'
              : ""
          }
				</div>
			</div>	
...
let generateMessageArea = (elem, chatIndex) => {
  ...
  document.getElementById("attach-files-user").href =
    "/attachImages/" + chat.contact.type + "/" + chat.contact.real_id + "/1";

  DOM.messageAreaDetails.innerHTML = `Created At ${mDate(
...
let sendRequestForMoreMessages = (chat, pageNo) => {
  if (
    (isLoading || chat.lastPageWas == chat.currentPage) &&
    chat.currentPage != 1
  ) {
    return false;
  }
  ...
  fetch(
    ...
    .catch((err) => {
      ...
      toastr["error"]("sorry, there are no results for your search", "error");
    });
};
