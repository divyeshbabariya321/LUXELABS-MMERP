...
function initialize_select2_single(selector, initial_skip = false) {
  const elem = $(selector);
  ...
  var multiple =
    typeof $(elem).attr("multiple") !== "undefined" ? true : false;
  ...
  options["minimumResultsForSearch"] =
    typeof $(elem).data("minimumresultsforsearch") !== "undefined"
...
function initialize_select2(initial_skip = false) {
  $(document)
    .find("select.globalSelect2:not(.initialized)")
