    ...
    if (!(isEntity === undefined)) {
      ...
      $.ajax({
        ...
        data: {
          ...
          keyword_or_question: keyword_or_question,
        },
        dataType: "json",
        success: function (response) {
      ...
      if (response.code == 200) {
        ...
        if (dialogBoxData.length > 0) {
          ...
          if (parent_id > 0) {
            ...
            html =
              "<tr class='parent_node_class_" +
              parent_id +
              "'><td style='padding:20px;' colspan='10'><table style='table-layout:fixed;' class='table table-striped table-bordered table-sm' cellspacing='0' width='100%'><tbody>" +
              html +
              "</tbody></table></td></tr>";
