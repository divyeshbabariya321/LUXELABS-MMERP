var page = {
  init: function (settings) {
    ...
    $(document).on("click", ".btn-get-translate-score", function (e) {
        e.preventDefault();
        page.getTranslatedTextScore($(this));
    });

    $(document).on("click", ".get-multi-score-btn", function (e) {
        e.preventDefault();
        page.getMultiScore($(this));
    });
  },
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/records",
      method: "get",
  ...
  deleteRecord: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/" + ele.data("id") + "/delete",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  push: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/" + ele.data("id") + "/push",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  pull: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/" + ele.data("id") + "/pull",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  findStores: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/" + ele.val() + "/get-stores",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  loadPage: function (ele) {
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/" + ele.val() + "/load-page",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  loadTranslation: function (ele) {
    ...
    var _z = {
      url:
        typeof href != "undefined"
          ? href
          : this.config.baseUrl + "/page/" + page + "/load-page",
      method: "get",
      data: {
        language: language,
      },
  ...
  translateForOtherLanguage: function (ele) {
    ...
    var _z = {
      url:
        this.config.baseUrl + "/page/" + page + "/translate-for-other-langauge",
      method: "get",
      beforeSend: function () {
        $("#loading-image").show();
      },
  ...
  afterPullPageInLive: function (response) {
    ...
    getTranslatedTextScore: function (ele) {
        ...
        this.sendAjax(_z, "afterGetTranslatedTextScore");
    },
    afterGetTranslatedTextScore: function (response) {
        if (response.code == 200) {
            toastr["success"](response.message, "");
            page.loadFirst();
        } else {
    ...
    getMultiScore: function(ele) {
        var numberOfChecked = $('.groups').filter(':checked').length;
        var totalCheckboxes = $('.groups').length;
        var numberNotChecked = totalCheckboxes - numberOfChecked;
        if (totalCheckboxes != numberNotChecked) {
            var searchIDs = $(".groups:checked").map(function(){
                return $(this).val();
            }).get();
            var _z = {
                url:
                    this.config.baseUrl +
                    "/page/get-multi-translated-text-score",
                    method: "post",
                    data: {ids: searchIDs},
                    beforeSend: function () {
                        $("#loading-image").show();
            ...
            this.sendAjax(_z, "afterGetMultiScore");
        }
    },
    afterGetMultiScore: function (response) {
        if (response.code == 200) {
            toastr["success"](response.message, "");
            page.loadFirst();
        } else {
...
$(document).on("click", ".keyword-list", function () {
  ...
  if (keywords) {
    if ($(this).hasClass("badge-green")) {
      ...
      $("#meta_keyword_avg_monthly").val(
        $("#meta_keyword_avg_monthly")
          .val()
          .replace("," + keywords + "-" + avg, "")
      );
    } else {
      $("#meta_keywords").val($("#meta_keywords").val() + "," + keywords);
      $("#meta_keyword_avg_monthly").val(
        $("#meta_keyword_avg_monthly").val() + "," + keywords + "-" + avg
      );
      $(this)
        .addClass("badge-green")
        .append('<i class="fa fa-remove pl-2"></i>');
    }
  }
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.length > 0) {
        ...
        if ($.isArray(response)) {
          $.each(response, function (index, data) {
            if ($.inArray(data.keyword, words) > -1) {
              t +=
                `<tr><td class="keyword-list badge-green" data-avg="` +
                data.avg_monthly_searches +
                `">` +
                data.keyword +
                `<i class="fa fa-remove pl-2"></i></td>`;
            } else {
              t +=
                `<tr><td class="keyword-list" data-avg="` +
                data.avg_monthly_searches +
                `" >` +
                data.keyword +
                `</td>`;
        ...
        $(document)
          .find("#extra-keyword-search-btn,#extra-keyword-search")
          .hide();
      }
    },
    complete: function () {
