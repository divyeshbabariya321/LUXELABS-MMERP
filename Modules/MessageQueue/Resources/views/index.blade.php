				  		...
				  		<div class="col">
						  	...
						  	<div class="form-group sending-number-section" style="display: none;">
							    ...
							    <select class="form-control" name="sending-number" id="sending-number">
								    @foreach(array_filter($allWhatsappNo) as $number => $apwCate)
				                        @if($number != "0")
					  			...
					  			@foreach(array_filter($allWhatsappNo) as $number => $apwCate)
			                        @if($number != "0")
				                        <div class="form-group">
										    ...
										    {{ html()->text("message_sending_limit[{$number}]", isset($sendingLimit[$number]) ? $sendingLimit[$number] : 0)->class("form-control message_sending_limit col-md") }}
									  	</div>
			                        @endif
			                    @endforeach
					  		</div>
					  	...
					  	<div class="row">
					  		<div class="col">
					  			<div class="form-group">
								    ...
								    {{ html()->input('time', "send_start_time", isset($sendStartTime) ? $sendStartTime : 0)->class("form-control message_sending_start")->attribute('datetime', "hh:mm") }}
							  	</div>
							  	<div class="form-group">
								    <label for="send_end_time">End Time:</label>
								    {{ html()->input('time', "send_end_time", isset($sendEndTime) ? $sendEndTime : 0)->class("form-control message_sending_end")->attribute('datetime', "hh:mm") }}
							  	</div>
							  	<div class="form-group">
							  		<label for="button">&nbsp;</label>
					  ...
					  <div class="row">
				  		<div class="col">
						  	...
						  	<div class="form-group">
                                ...
                                {{ html()->text('to', request('to'))->class('form-control')->placeholder('Enter Number to') }}
                            </div>
						  	<div class="form-group">
							    <label for="action">Group:</label>
                                {{ html()->select('group_id', $groupList)->class('form-control select2')->placeholder('Group')->value(request('group_id')) }}
                            </div>
						  	<div class="form-group">
                                ...
                                {{ html()->select('limit', [
                                    ...
                                    500 => '500',
                                    1000 => '1000',
                                ])->class('form-control select2')->placeholder('Page limit')->value(request('limit')) }}
                            </div>
						  	<div class="form-group">
							    <label for="action">Only Lead:</label>
                                {{ html()->select('only_lead', [
                                    null => '-- ALL --',
                                    1 => 'Yes',
                                ])->class('form-control select2')->value(request('only_lead')) }}
                            </div>
						  	<div class="form-group">
							    <label for="action">Communicated:</label>
                                {{ html()->select('communicated', [
                                    'yes' => 'Yes',
                                    'no' => 'No',
                                ])->class('form-control select2')->placeholder('Communicated')->value(request('communicated')) }}
                            </div>
						  	<div class="form-group">
						  		<label for="button">&nbsp;</label>
				  			...
				  			@foreach(array_filter($allWhatsappNo) as $number => $apwCate)
		                        @if($number != "0")
			                        <div class="form-group">
									    ...
									    {{ html()->text("message_sending_time[{$number}]", isset($sendingTime[$number]) ? $sendingTime[$number] : 0)->class("form-control message_sending_time col-md") }}
								  	</div>
		                        @endif
		                    @endforeach
		                    <div class="form-group">
