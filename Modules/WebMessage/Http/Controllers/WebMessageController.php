...
class WebMessageController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $jsonAutoSuggest = AutoReply::get()->pluck(['reply'])->toArray();
    ...
    {
        $blockedUser = BlockWebMessageList::all();
        ...
        $customerList = ChatMessage::where(function ($q) {
            ...
            ->orderByDesc('last_chat_id')
            ->get();

        // need to setup list as per the the customer, supplier, vendor etc
        $vendorList = ChatMessage::where(function ($q) {
            $q->whereNull('group_id')->orWhere('group_id', 0);
        })
        ...
        // need to setup list as per the the customer, supplier, vendor etc
        $supplierList = ChatMessage::where(function ($q) {
            $q->whereNull('group_id')->orWhere('group_id', 0);
        })
    ...
    public function messageList(Request $request, $id): JsonResponse
    {
        $params = $request->all();
        ...
        if (! empty($customer)) {
            ...
            $mediables = Mediables::whereIn('media_id', $allMediaIds)->where('mediable_type', Product::class)->get();
    ...
    public function send(Request $request)
    {
        ...
        switch ($case) {
            case 'customer':
                ...
                return app(WhatsAppController::class)->sendMessage($request, 'customer', true);
                break;
            case 'vendor':
                $params = [
                ...
                return app(WhatsAppController::class)->sendMessage($request, 'vendor', true);
                break;
            case 'supplier':
                $params = [
                ...
                return app(WhatsAppController::class)->sendMessage($request, 'supplier', true);
                break;
            default:
                // code...
                break;
    ...
    public function userAction(Request $request): JsonResponse
    {
        ...
        if (! empty($pureValue['object']) && ! empty($pureValue['real_id'])) {
            switch ($params['case']) {
                case 'block':
                    $bml = BlockWebMessageList::updateOrCreate(
