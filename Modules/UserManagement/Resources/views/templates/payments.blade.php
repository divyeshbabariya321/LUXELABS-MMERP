    ...
    <table id="payment-table" class="table table-bordered" style="visibility: hidden;">
        ...
        <tbody>
            <tr style="position: relative">
                <td onclick="toggle('{{$id}}')" style="cursor: pointer;">
                    <div style="margin-bottom: 20px;">
                        <span href="#{{$id}}-expandable">@if(!empty($user)) {{$user->name}} @endif</span>
                    </div>
                    <div id="elastic-{{$id}}" class="activity-container" style="height: 500px;" data-expanded="false">
                ...
                <td>@if(!empty($user)) {{round($user->secondsTracked / 3600, 2) }} @endif</td>
                <td>@if(!empty($user)) {{isset($user->currentRate) ? $user->currentRate->hourly_rate : '-'}} @endif</td>
                <td>@if(!empty($user)) {{$user->currency}} @endif</td>
                <td>@if(!empty($user)) {{$user->total}} @endif</td>
                <td>@if(!empty($user)) {{$user->balance}} @endif</td>
                <td>
                @if(!empty($user))
                    <button class="btn btn-secondary" onclick="makePayment({{$id}})">Pay</button>
                @endif
                </td>
...
<div id="paymentModal" class="modal fade" role="dialog">
    <div class="modal-dialog" style="margin-top: 150px">
        ...
        <div class="modal-content">
            {{ html()->form('POST', url('/hubstaff/makePayment'))->open() }}
            {{ html()->hidden('user_id', Input::old('user_id')) }}
            <div class="modal-body">
                <div class="form-group">
                    {{ html()->label('Amount', 'amount') }}
                    {{ html()->number('amount')->class('form-control') }}
                </div>
                <div class="form-group">
                    {{ html()->label('Currency', 'currency') }}
                    {{ html()->text('currency')->class('form-control') }}
                </div>
                <div class="form-group">
            ...
            <div class="modal-footer">
                {{ html()->submit('Pay')->class('btn btn-primary') }}
...
<script>
    ...
    $(function() {
        ...
        weekpicker.find("input").on("dp.change", function() {
            window.location = '/user-management/payments/'+id+'?week=' + weekpicker.getWeek() + '&year=' + weekpicker.getYear();
        });
    });
