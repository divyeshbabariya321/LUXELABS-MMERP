...
<div class="row">
  <div class="col-xs-12 col-md-4 border">
    {{ html()->modelForm($user, 'PATCH', route('user-management.update', [$user->id]))->class('my-3')->open() }}
         ...
         $(document).on('click', '.send-communication', function(e) {
           ...
           if ($(this).closest('form')[0].checkValidity()) {
             $.ajax({
               ...
               toastr["error"]("Error sending a message", "error");
      ...
      $(document).on('click', '.change_message_status', function(e) {
        ...
        if ($(this).hasClass('wa_send_message')) {
          ...
          $.ajax({
            ...
            toastr["error"]("Could not create whatsapp message", "error");
          });
        }
          $.ajax({
            url: url,
            ...
            toastr["error"]("Could not change status", "error");
          });
