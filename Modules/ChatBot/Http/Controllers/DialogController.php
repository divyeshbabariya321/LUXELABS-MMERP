...
class DialogController extends Controller
{
    ...
    public function dialogGrid(): View
    {
        $allSuggestedOptions = ChatbotDialog::allSuggestedOptions();
    ...
    public function restDetails(Request $request, $id): JsonResponse
    {
        $details = [];
    ...
    public function restCreate(Request $request): JsonResponse
    {
        $params = [
    ...
    public function restStatus(Request $request): JsonResponse
    {
        //        dd('jhkj');
        $parentId = $request->get('parent_id', 0);
        ...
        $chatDialog = ChatbotDialog::leftJoin('chatbot_dialog_responses as cdr', 'cdr.chatbot_dialog_id', 'chatbot_dialogs.id')
            ...
            ->orderBy('chatbot_dialogs.previous_sibling');
        ...
        if (! empty($chatDialogArray)) {
            foreach ($chatDialogArray as $k => $chatDlg) {
                ...
                if ($chatDlg['parent_id'] == 0 && $chatDlg['dialog_response'] == null) {
                    ...
                    $response = ChatbotDialog::where('parent_id', $chatDlg['id'])->select('match_condition')->with('singleResponse')->pluck('match_condition')->toArray();
                    $str = '';
        ...
        $question = ChatbotQuestion::where('keyword_or_question', 'intent')->selectRaw("concat('#','',value) as value")->pluck('value', 'value')->toArray();
        $keywords = ChatbotQuestion::where('keyword_or_question', 'entity')->selectRaw("CONCAT('@', '', value) as value")->pluck('value', 'value')->toArray();

        $allSuggestedOptions = $keywords + $question;
        //dd($chatDialog);
        foreach ($chatDialog as $k => $dialogNode) {
            $childNodeCount = ChatbotDialog::select('id', 'parent_id')->where('parent_id', $dialogNode['id'])->get();
    ...
    public function restDelete(Request $request, $id): JsonResponse
    {
        $chatbotDialog = ChatbotDialog::find($id);
        if (! empty($chatbotDialog)) {
            // check if it has any parent
    ...
    public function localErrorLog(Request $request): View
    {
        // dd($request->all());
    ...
    public function getWebsiteResponse(Request $request): JsonResponse
    {
        $details = [];
    ...
    public function getAllResponse($id): View
    {
    ...
    public function submitResponse($id, Request $request): JsonResponse
    {
