...
<div class="row">
  <div class="col-lg-12 margin-tb">
    ...
    <div class="pull-right mt-4">
      <a class="btn btn-xs btn-secondary" href="{{ route('lawyer.index') }}">Back</a>
...
{{--@include('lawyer.partials.product-modals')--}}

<div id="exTab2" class="container">
  <ul class="nav nav-tabs">
    <li class="active">
      <a href="#info-tab" data-toggle="tab">Lawyer Info</a>
    </li>
   {{-- <li>
...
<div class="row">
        ...
        <div class="row">
          <div class="col-xs-12">
            <div class="form-group">
              <select class="form-control input-sm" name="category_id" id="lawyer_category">
                <option value="">Select an Speciality</option>

                @foreach ($specialities as $speciality)
                  <option value="{{ $speciality->id }}" {{ $speciality->id == $lawyer->speciality_id ? 'selected' : '' }}>{{ $speciality->title }}</option>
                @endforeach
              </select>
            </div>

            <div class="form-group form-inline">
              <input type="text" name="name" id="lawyer_name_name" class="form-control input-sm" placeholder="Lawyer" value="{{ $lawyer->name }}">
            ...
            <div class="form-group form-inline">
              <input type="number" id="lawyer_phone" name="phone" class="form-control input-sm" placeholder="910000000000" value="{{ $lawyer->phone }}">
            </div>

            <div class="form-group">
              <select class="form-control input-sm" name="default_phone" id="lawyer_default_phone">
                <option value="">Select Default Phone</option>
                @if ($lawyer->phone != '')
                  <option value="{{ $lawyer->phone }}" {{ $lawyer->phone == $lawyer->default_phone ? 'selected' : '' }}>{{ $lawyer->phone }} - Lawyer's Phone</option>
                @endif

                @if ($lawyer->cases)
                  @foreach ($lawyer->cases as $agent)
                    <option value="{{ $agent->phone }}" {{ $agent->phone == $lawyer->default_phone ? 'selected' : '' }}>{{ $agent->phone }} - {{ $agent->name }}</option>
                  @endforeach
                @endif
              </select>
            </div>
            ...
            <div class="form-group">
              <select name="whatsapp_number" id="lawyer_whatsapp_number" class="form-control input-sm">
                <option value>Whatsapp Number</option>
                <option value="971569119192" {{ '971569119192' == $lawyer->whatsapp_number ? ' selected' : '' }}>971569119192 Indian</option>
                <option value="971502609192" {{ '971502609192' == $lawyer->whatsapp_number ? ' selected' : '' }}>971502609192 Dubai</option>
              </select>
            </div>

            <div class="form-group">
              <textarea name="address" id="lawyer_address" class="form-control input-sm" rows="3" cols="80" placeholder="Address">{{ $lawyer->address }}</textarea>
            </div>

              <div class="form-group">
                <input type="email" name="email" id="lawyer_email" class="form-control input-sm" placeholder="Email" value="{{ $lawyer->email }}">
              </div>

            <div class="form-group">
              <button type="button" id="updateLawyerButton" class="btn btn-xs btn-secondary">Save</button>
            </div>
          </div>
  ...
  <div class="col-xs-12 col-md-4 mb-3">
    <div class="border">
      <form action="{{ route('whatsapp.send', 'lawyer') }}" method="POST" enctype="multipart/form-data">
        <div class="d-flex">
          @csrf
          ...
          <div class="form-group flex-fill mr-3">
            <button type="button" id="lawyerMessageButton" class="btn btn-image"><img src="/images/support.png" /></button>
            <textarea  class="form-control mb-3 hidden" style="height: 110px;" name="body" placeholder="Received from Lawyer"></textarea>
  ...
  <script type="text/javascript">
    ...
    $(document).on('click', ".collapsible-message", function() {
      ...
      if (selection.toString().length === 0) {
            ...
            if ( current.get( 0 ) ) {
              return false;
               ...
               if (message.status == 0 || message.status == 5 || message.status == 6) {
                 var meta = $("<em>Lawyer " + moment(message.created_at).format('DD-MM H:mm') + " </em>");
        ...
        const socket = io("https://sololuxury.co/?realtime_id=lawyer_{{ $lawyer->id }}");
      ...
      $('#lawyerMessageButton').on('click', function() {
        $(this).siblings('textarea').removeClass('hidden');
        $(this).addClass('hidden');
      });

      $('#updateLawyerButton').on('click', function() {
        var id = {{ $lawyer->id }};
        var thiss = $(this);
        var name = $('#lawyer_name_name').val();
        var category = $('#lawyer_category').val();
        var phone = $('#lawyer_phone').val();
        var default_phone = $('#lawyer_default_phone').val();
        var whatsapp_number = $('#lawyer_whatsapp_number').val();
        var address = $('#lawyer_address').val();
        var email = $('#lawyer_email').val();
        var social_handle = $('#lawyer_social_handle').val();
        var website = $('#lawyer_website').val();
        var login = $('#lawyer_login').val();
        var password = $('#lawyer_password').val();
        var gst = $('#lawyer_gst').val();
        var account_name = $('#lawyer_account_name').val();
        var account_iban = $('#lawyer_account_iban').val();
        var account_swift = $('#lawyer_account_swift').val();
      ...
      $(document).on('click', '.edit-product', function() {
        ...
        var url = "{{ url('lawyer/product') }}/" + product.id;

        $('#productEditModal form').attr('action', url);
        $('#lawyer_lawyer_id').val(product.lawyer_id);
        $('#lawyer_date_of_order').val(product.date_of_order);
        $('#lawyer_name').val(product.name);
        $('#lawyer_qty').val(product.qty);
        $('#lawyer_price').val(product.price);
        $('#lawyer_payment_terms').val(product.payment_terms);
        $('#lawyer_recurring_type option[value="' + product.recurring_type + '"]').prop('selected', true);
        $('#lawyer_delivery_date').val(product.delivery_date);
        $('#lawyer_received_by').val(product.received_by);
        $('#lawyer_approved_by').val(product.approved_by);
        $('#lawyer_payment_details').val(product.payment_details);
      });

      $('#date-of-order, #lawyer-date-of-order, #delivery-date, #lawyer-delivery-date').datetimepicker({
