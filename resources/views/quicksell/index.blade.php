...
<form action="{{ route('quicksell.search') }}" method="GET" id="searchForm" class="form-inline align-items-start">
  ...
  <div class="form-group mr-3">
    @php $brands =  $brands_arr; @endphp
    {{ html()->multiselect('brand[]', $brands, request('brand'))->class("form-control select-multiple2")->data('placeholder', "Select Brands") }}
  </div>

  <div class="form-group mr-3">
    @php $colors = (new \App\Colors())->all(); @endphp
    {{ html()->multiselect('color[]', $colors, request('color'))->class("form-control select-multiple2")->data('placeholder', "Select Colors")->style("min-width:250px;") }}
  </div>

  <div class="form-group mr-3">
    {{ html()->multiselect('supplier[]', $suppliers->pluck('supplier', 'id'), request('supplier'))->class("form-control select-multiple2")->data('placeholder', "Select supplier") }}
  </div>

  @if (Auth::user()->hasRole('Admin'))
    <div class="form-group mr-3">
      {{ html()->multiselect('location[]', $locations, request('location'))->class("form-control select-multiple2")->data('placeholder', "Select location") }}
    </div>
  @endif
  ...
  <div class="form-group mr-3">
    {{ html()->multiselect('group[]', $groups->pluck('name','id'), request('group'))->class("form-control select-multiple2")->data('placeholder', "Select Group")->style("min-width:250px;") }}
  </div>

  <div class="form-group mr-3">
    {{ html()->select('per_page', ["20" => "20 Images Per Page", "30" => "30 Images Per Page", "50" => "50 Images Per Page", "100" => "100 Images Per Page"], request()->get("per_page", null))->placeholder('-- Select Images Per Page --')->class('form-control') }}
  </div>
  <div class="form-group mr-3">
...
<div class="infinite-scroll">
  <div class="row mt-6 " style="margin: 10px;">
    ...
    <div class="col-md-3 col-xs-6 text-left">
      ...
      <img src="{{ $product->getMedia($media_tags)->first()
                ? getMediaUrl($product->getMedia($media_tags)->first())
      ...
      <p class="groups-css">Group :@if($product->groups->count() == 0) <input type="checkbox" name="blank" class="group-checkbox checkbox" data-id="{{ $product->id }}"> @else @foreach($product->groups as $group)
        @php 
      $grp = $groups->where('group',$group->quicksell_group_id)->first();
      @endphp 
      ...
      {{ html()->form('POST', route('products.archive', [$product->id]))->style('display:inline')->open() }}
      <button type="submit" class="btn btn-image"><img src="/images/archive.png" /></button>
      {{ html()->form()->close() }}

       @if(auth()->user()->isAdmin())
      {{ html()->form('DELETE', route('products.destroy', [$product->id]))->style('display:inline')->open() }}
