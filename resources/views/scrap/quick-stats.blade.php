...
@section('large_content')
    ...
    <form class="" action="/scrap/quick-statistics">
        <div class="row">
            <div class="form-group col-md-3">
                <select class="form-control" name="term" id="supplier_filter" placeholder="Select Supplier name">
                    <option></option>
                    @if(count($selectedSupplier) > 0)
                        <option value="{{$selectedSupplier['id']}}" selected>{{$selectedSupplier['name']}}</option>
                    @endif
                </select>
                <a href="{{ route('statistics.quick') }}" class="btn btn-xs d-inline reset-filter">
                    <i class="fa fa-eraser"></i>
                </a>
                <button type="submit" class="btn btn-xs d-inline">
   ...
   <?php $totalCountedUrl = 0; ?>
    ...
    ?>
                            ...
                            <td>
                                <div class="form-group">
                                    {{ html()->select("auto_restart", [0 => "Off", 1 => "On"], $supplier->auto_restart)->class("form-control auto_restart select2")->style("width:100%;") }}
                                </div>
                            </td>
                            ...
                            <td>{{ !empty($data) ? $data->total_new_product : '' }}</td>

                            <td data-start-time="@if($supplier->last_started_at){{$supplier->last_started_at }}@endif" data-end-time="@if($supplier->last_completed_at){{$supplier->last_completed_at }}@endif" class="show-scraper-detail">
    ...
    ?>
                            ...
                            <td>
                                <div class="form-group">
                                    {{ html()->select("full_scrape", [0 => "No", 1 => "Yes"], $supplier->full_scrape)->class("form-control full_scrape select2")->style("width:100%;") }}
                                </div>
                            </td>
                            <td>
                                <button type="button" class="btn btn-xs toggle-class bg-transparent" data-id="{{ $supplier->id }}" title="Expand more data">
                                    <i class="fa fa-forward"></i>
                                </button>
                                <a class="btn btn-xs bg-transparent" href="{{ get_server_last_log_file($supplier->scraper_name,$supplier->server_id) }}" id="link" target="-blank" title="View log">
                                    <i class="fa fa-eye"></i>
                                </a>
                                <button type="button" class="btn btn-xs bg-transparent" onclick="restartScript('{{ $supplier->scraper_name }}' , '{{ $supplier->server_id }}' )" title="Restart script">
                                    <i class="fa fa-refresh"></i>
                                </button>

                                <button type="button" class="btn btn-xs bg-transparent" onclick="getRunningStatus('{{ $supplier->scraper_name }}' , '{{ $supplier->server_id }}' )" title="Check running status">
                                ...
                                @if($isAdmin)
                                    <div class="flag-scraper-div d-inline"> 
                                        @if ($supplier->flag == 1)
                                            <button type="button" class="btn btn-xs flag-scraper bg-transparent" data-flag="0" data-id="{{ $supplier->id }}">
                                                <i class="fa fa-flag" style="color: red;"></i>
                                            </button>
                                        @else
                                            <button type="button" class="btn btn-xs flag-scraper bg-transparent" data-flag="1" data-id="{{ $supplier->id }}">
                                                <i class="fa fa-flag"></i>
                                            </button>
                                        @endif
                                    </div>
                                    <div class="flag-scraper-developer-div d-inline"> 
                                        @if ($supplier->developer_flag == 1)
                                            <button type="button" class="btn btn-xs flag-scraper-developer bg-transparent" data-flag="0" data-id="{{ $supplier->id }}">
                            ...
                            <tr class="hidden_row_{{ $supplier->id  }} dis-none" data-eleid="{{ $supplier->id }}">
                                ...
                                <td colspan="1">
                                    ...
                                    <div class="input-group">
                                        {{ html()->select("start_time", ['' => "--Time--"] + $timeDropDown, $supplier->scraper_start_time)->class("form-control start_time select2")->style("width:100%;") }}
                                    </div>
                                </td>
                                <td colspan="2">
                                    <label>Made By:</label>
                                ...
                                <td colspan="2">
                                    <label>Type:</label>
                                    <div class="form-group">
                                        {{ html()->select("scraper_type", ['' => '-- Select Type --'] + \App\Helpers\DevelopmentHelper::scrapTypes(), $supplier->scraper_type)->class("form-control scraper_type select2")->style("width:100%;") }}
                                    </div>
                                </td>
                                <td colspan="2">
                                    <label>Parent Scrapper:</label>
                                    <div class="form-group">
                                        {{ html()->select("parent_supplier_id", [0 => "N/A"] + $allScrapperName, $supplier->parent_supplier_id)->class("form-control parent_supplier_id select2")->style("width:100%;") }}
                                    </div>
                                </td>
                                <td colspan="2">
                                    <label>Next Step:</label>
                                    <div class="form-group">
                                        {{ html()->select("next_step_in_product_flow", [0 => "N/A"] + \App\Helpers\StatusHelper::getStatus(), $supplier->next_step_in_product_flow)->class("form-control next_step_in_product_flow select2")->style("width:100%;") }}
                                    </div>
                                </td>
                                <td colspan="2">
                            ...
                            @if($supplier->childrenScraper)
                                @if($supplier->children_scraper_count != 0)
                                    ...
                                    <td width="4%">
                                        <div class="form-group">
                                            {{ html()->select("auto_restart", [0 => "Off", 1 => "On"], $childSupplier->auto_restart)->class("form-control auto_restart select2")->style("width:100%;") }}
                                        </div>
                                    </td>
                                    ...
                                    <td width="6%">
                                        <div class="form-group">
                                            {{ html()->select("full_scrape", [0 => "No", 1 => "Yes"], $childSupplier->full_scrape)->class("form-control full_scrape select2")->style("width:100%;") }}
                                        </div>
                                    </td>                                    
                                    ...
                                    <tr class="hidden_row_{{ $childSupplier->id  }} dis-none" data-eleid="{{ $childSupplier->id }}">
                                        ...
                                        <td colspan="3">
                                            <label>Start Time:</label>
                                            <div class="input-group">
                                                {{ html()->select("start_time", ['' => "--Time--"] + $timeDropDown, $childSupplier->scraper_start_time)->class("form-control start_time select2")->style("width:100%;") }}
                                            </div>
                                        </td>
                                        <td colspan="3">
                                            <label>Made By:</label>
                                            ...
                                            {{ html()->select("scraper_made_by", $madeByArray, $childSupplier->scraper_made_by)->class("form-control scraper_made_by globalSelect2")->style("width:100%;")->data('ajax', route('select2.user'))->data('placeholder', 'Made by') }}
                                            </div>
                                        </td>
                                        <td colspan="2">
                                            <label>Type:</label>
                                            <div class="form-group">
                                                {{ html()->select("scraper_type", ['' => '-- Select Type --'] + \App\Helpers\DevelopmentHelper::scrapTypes(), $childSupplier->scraper_type)->class("form-control scraper_type select2")->style("width:100%;") }}
                                            </div>
                                        </td>
                                        <td colspan="2">
                                            <label>Parent Scrapper:</label>
                                            <div class="form-group">
                                                {{ html()->select("parent_supplier_id", [0 => "N/A"] + $allScrapperName, $childSupplier->parent_supplier_id)->class("form-control parent_supplier_id select2")->style("width:100%;") }}
                                            </div>
                                        </td>
                                        <td colspan="2">
                                            <label>Next Step:</label>
                                            <div class="form-group">
                                                {{ html()->select("next_step_in_product_flow", [0 => "N/A"] + \App\Helpers\StatusHelper::getStatus(), $childSupplier->next_step_in_product_flow)->class("form-control next_step_in_product_flow select2")->style("width:100%;") }}
                                            </div>
                                        </td>
                                        <td colspan="2">
                                            <label>Server Id:</label>
                                            <div class="form-group">
                                                {{ html()->text("server_id", $childSupplier->server_id)->class("form-control server-id-update") }}
                ...
                <form action="{{ route('save.childrenScraper') }}" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                {{ html()->select("start_time", ['' => "--Time--"] + $timeDropDown, '')->class("form-control start_time select2")->style("width:100%;") }}
                            </div>
                        </div>
                        <div class="form-group">
                            <strong>Made By:</strong>
                            <div class="form-group">
                                {{ html()->select("scraper_made_by", ["" => "N/A"] + $users, '')->class("form-control scraper_made_by select2")->style("width:100%;") }}
                            </div>
                        </div>
                        <div class="form-group">
            ...
            $.ajax({
                ...
                beforeSend: function () {
                 ...
                 if(response.data.flag == 1) {
                    $this.closest(".flag-scraper-div").append('<button type="button" class="btn btn-xs flag-scraper bg-transparent" data-flag="0" data-id="'+response.data.supplier_id+'"><i class="fa fa-flag" style="color: red;"></i></button>');
                 }else{
                    $this.closest(".flag-scraper-div").append('<button type="button" class="btn btn-xs flag-scraper bg-transparent" data-flag="1" data-id="'+response.data.supplier_id+'"><i class="fa fa-flag"></i></button>');
                 }
                 $this.remove();
            }).fail(function() {
                $("#loading-image").hide();
