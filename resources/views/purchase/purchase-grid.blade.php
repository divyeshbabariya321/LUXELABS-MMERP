...
@section('content')
        ...
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <form action="{{url()->current()}}" method="GET" class="form-inline align-items-start">
                    ...
                    <div class="form-group mr-3">
                        {{ html()->select('supplier[]', $suppliers_array, !empty(request()->get('supplier')[0]) ? request()->get('supplier')[0] : '')->placeholder('Select a Supplier')->class('form-control select-multiple2') }}
                    </div>
                    ...
                    <div class="form-group mr-3">
                        {{ html()->select("customer_id", [], request()->get('customer', null))->class("form-control customer-search-box")->placeholder('Select a Customer')->style("width:100%;") }}
                    </div>
            ...
            <table class="table table-bordered">
                ...
                @foreach ($products as $product)
                    ...
                    <tr>
                        ...
                        @php
                            $suppliersArray = [];
                            $data = getScrappedProduct($product['sku']);
                            $cnt = count($data);
                        @endphp
                        @if($cnt > 0)
                            @php
                                $suppliersArray = getSupplierArr($product);
                            @endphp
                        @endif
                        <td>
                            @if($custcount > 1)
    ...
    <script>
        ...
        // };
        //
        // var suppliers_array = @json($suppliers_array);
        // const products = [
        //         @foreach ($products as $product)
        //
        //     {
        ...
        //
        //         let html = '<form action="{{ route('purchase.store') }}" method="POST">@csrf<input type="hidden" name="purchase_handler" value="{{ Auth::id() }}" /><input type="hidden" name="supplier_id" value="' + key + '" />';
