    ...
    <tbody>
            ...
            <td colspan="15">
                <div class="row">
                    <div class="col-md-3">
                        ...
                        @php
                            $logScrapers = array();
                            $descriptions = \App\Helpers\DevelopmentHelper::getScrapedProductsWhereSku($product->sku);
                        @endphp
        ...
        @php
            ...
            if(!empty($product->listing_remark)){
                $status_color = \App\Helpers\DevelopmentHelper::getProductListingFinalStatus($product->listing_remark);
            }       
        @endphp
        @if(!empty($dynamicColumnsToShowPlf))
                            ...
                            @foreach ( $product->log_scraper_vs_ai as $resultAi )
                                ...
                                @if ( !empty($resultAi->category) )
                                    <button id="ai-category-{{ $product->id }}" data-id="{{ $product->id }}"
                                            data-category="{{ \App\Helpers\DevelopmentHelper::getCategoryIdByKeyword($resultAi->category, $resultAi->gender, null) }}"
