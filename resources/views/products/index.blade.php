...
<div class="table-responsive">
  <table class="table table-bordered">
    ...
    <tr>
      <td>{{ $product->id }}</td>
      <td>
        @if ($images = $product->getMedia($mediaTags))
      ...
      <td>{{ $product->name }}</td>
      <td>
         {{ html()->multiselect("is_published[{$product->id}][]", $websiteList, $product->publishedOn())->class("form-control select2-mul update-product-store")->data('id', $product->id) }}
      </td>
      <td>
        ...
        @if (isset($archived) && $archived == 'true')
          {{ html()->form('POST', route('products.restore', [$product->id]))->style('display:inline')->open() }}
