        ...
        <div class="col-lg-12 margin-tb">
            <h2 class="page-heading">Approved Product Listing ({{ $products_count }}) <a
                ...
                <div class="row">
                    ...
                    <div class="col-sm-2">
                        <div class="form-group">
                            @if(auth()->user()->isReviwerLikeAdmin('final_listing'))
                                {{ html()->checkbox("submit_for_approval", (bool) (request('submit_for_approval') == "on"), "on")->class("") }}
                    ...
                    <div class="col-sm-1">
                        <div class="form-group">
                           {{ html()->checkbox("without_title", (bool) (request('without_title') == "on"), "on")->class("") }}
                                <lable for="without_title pr-3">No title</lable>
                        </div>
                    </div>
                    <div class="col-sm-1">
                        <div class="form-group">
                           {{ html()->checkbox("without_size", (bool) (request('without_size') == "on"), "on")->class("") }}
                                <lable for="without_size pr-3">No size</lable>
                        </div>
                    </div>
                    <div class="col-sm-1">
                        <div class="form-group">
                           {{ html()->checkbox("without_composition", (bool) (request('without_composition') == "on"), "on")->class("") }}
                            ...
                            <td colspan="15">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        if ($product->brands) {
                                            $segmentPrice = \App\Helpers\DevelopmentHelper::getBrandSegmentPrice($product->brands->brand_segment, $product->category);
                                            if ($segmentPrice) {
                                        ...
                                        <br/>
                                        @php
                                            $descriptions = \App\Helpers\DevelopmentHelper::getScrapedProducts($product->sku);
                                        @endphp
                                        ...
                                        <p class="same-color">
                                            View All:
                                            <strong>{{ isset($product->product_category->id) ? \App\Helpers\DevelopmentHelper::getCategoryPathById($product->product_category->id)  : '' }}</strong>
                                            <br/>
                                            View All:
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        @php
                                            $logScrapers = \App\Helpers\DevelopmentHelper::getValidatedScrapedProducts($product->sku);
                                        @endphp
                                        @if ($logScrapers)
                                            <div>
                                                <br/>
        ...
        <div id="product_image_{{ $product->id }}" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                ...
                <div class="modal-content ">
                    ...
                    <div class="modal-body">
                    @php
                        $anyCropExist = \App\Helpers\DevelopmentHelper::getSiteCroppedImageAnyCropExists($product->id);
                    @endphp
                    <button type="button" value="reject" id="reject-all-cropping{{$product->id}}" data-product_id="{{$product->id}}" class="btn btn-xs btn-secondary pull-right reject-all-cropping">
                        ...
                        @if(!empty($websiteList))
                            @foreach($websiteList as $index => $site)
                                @php 
                                    $siteCroppedImage = \App\Helpers\DevelopmentHelper::getSiteCroppedImage($product->id,$site->id);
                                @endphp
                                ...
                                <div class="row">
                                    <div class="col-md-5">
                                        ...
                                        if (file_exists($media->getAbsolutePath())) {
                                            [$width, $height] = getimagesize($media->getAbsolutePath());
                                        ...
                                        $gridImage = \App\Helpers\DevelopmentHelper::getCroppingGridImageByCategoryId($product->category);
                                            ...
                                            ?>
                                                    <div class="thumbnail-pic">
                                                        ...
                                                        <img style="display:block; width: 70px; height: 80px; margin-top: 5px;"
                                                             ...
                                                             title="ID: {{ $product->id }}"
                                                             onclick="replaceThumbnail('{{ $product->id }}','{{ getMediaUrl($media) }}','{{$gridImage}}','{{ $site->id }}')">
                                                    </div>
                                                    <?php } ?>
                                                @endif
                                            @endforeach
                                    ...
                                    <div class="col-md-7" id="col-large-image{{ $product->id }}{{$site->id}}">
                                        @if ($product->hasMedia($tag))
                                            @php $siteImage =  getMediaUrl($product->getMedia($tag)->first()) @endphp
        ...
        <div id="description_modal_view_{{ $product->id }}" class="modal fade" role="dialog">
            <div class="modal-dialog">
                ...
                <div class="modal-content ">
                    ...
                    @php
                    $product = getProduct($product->id);
                    $attributes = \App\Helpers\DevelopmentHelper::getStoreWebsiteProductAttribute($product->id);
                    @endphp
...
@section('scripts')
    ...
    <script type="text/javascript">
        var categoryJson = @json($category_array);
        ...
        var category_tree = @json($category_tree);
        var categories_array = @json($categories_array);
        var id_list = {
