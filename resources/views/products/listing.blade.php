                ...
                <table class="table table-bordered table-striped" style="max-width: 100% !important;">
                        ...
                        <tr id="product_{{ $product->id }}">
                            @if (!Auth::user()->hasRole('ImageCropers'))
                                <td style="word-break: break-all; word-wrap: break-word">
                                    ...
                                    @php $special_product =getProduct($product->id) @endphp
                                    @if ($special_product->hasMedia($media_tags))
                                        <a href="{{ route('products.show', $product->id) }}" target="_new">
                                            <img src="{{ getMediaUrl($special_product->getMedia($media_tags)->first()) }}" class="quick-image-container img-responive" style="width: 70px;" alt="" data-toggle="tooltip" data-placement="top" title="ID: {{ $product->id }}">
                                        </a>
                                    @else
                                    ...
                                    <div>
                                        @if ($special_product->hasMedia($media_tags))
                                            @foreach($special_product->getMedia($media_tags) as $media)
                                                <a href="{{ getMediaUrl($media) }}" target="_new">
                                                    <img style="width: 50px;" src="{{ getMediaUrl($media) }}" class="quick-image-container img-responive" alt="" data-toggle="tooltip" data-placement="top" title="ID: {{ $product->id }}">
                                                </a>
                                            @endforeach
                                        @endif
                                    </div>
                  ...
                  <textarea name="description" class="form-control quick-description-edit-textarea hidden" rows="8" cols="80">{{ $product->short_description }}</textarea>
                                ...
                                <td class="table-hover-cell">
                                    ...
                                    @php
                                        // Get log results
                                        $logScraperVsAi = \App\Helpers\DevelopmentHelper::getLogScraperVsAiWhere($product->id);
                  ...
                  <input type="number" name="dmeasurement" class="form-control mt-1" placeholder="Depth" min="0" max="999" value="{{ $product->dmeasurement }}">
                                ...
                                <td>
                                    ...
                                    @if ($product->product_user_id != null)
                                        <br>{{ \App\Helpers\DevelopmentHelper::getUserName($product->product_user_id) }}
                                    @endif
                                ...
                                <td>
                                    ...
                                    @if ($special_product->hasMedia($media_tags))
                                        <a href="{{ route('products.show', $product['id']) }}" target="_blank">
                                            <img src="{{ getMediaUrl($special_product->getMedia($media_tags)->first()) }}" class="quick-image-container img-responive" style="width: 100px;" alt="" data-toggle="tooltip" data-placement="top" title="ID: {{ $product['id'] }}">
                                        </a>
                                    @else
                  ...
                  <span class="description-container">{{ $product->short_description }}</span>
                                ...
                                <td>
                                    ...
                                    @if ($product->product_user_id != null)
                                        {{ \App\Helpers\DevelopmentHelper::getUserName($product->product_user_id) }}
                                    @endif
                                </td>
