    ...
    <div class="row">
        <table class="mt-5 table table-striped table-bordered">
            ...
            @foreach($histories as $key=>$history)
                @if($key==0)
                    @php $lastUser = $history->user_id; $balance = 0; $listingRates = \App\Helpers\DevelopmentHelper::getUserListingRates($history->user_id);$car = $listingRates['listing_approval_rate']; $cjr = $listingRates['listing_rejection_rate'];  @endphp
                @endif
                @if ($lastUser!=$history->user_id)
                    ...
                    @php $balance = 0; $ta = 0; $tr = 0; $listingRates = \App\Helpers\DevelopmentHelper::getUserListingRates($history->user_id);$car = $listingRates['listing_approval_rate']; $cjr = $listingRates['listing_rejection_rate']; @endphp
                @endif
                <tr>
                    <td>{{ $history->date }}</td>
                ...
                <tr>
                    ...
                    @php
                    $pays = \App\Helpers\DevelopmentHelper::getListingPayments($history->user_id,$history->date);
                    @endphp
                    @foreach($pays as $pay)
