        ...
        <div class="modal-content ">
            ...
            <div class="modal-body">
                ...
                <div class="col-md-5">
                    ...
                    <?php $gridImage = ''; ?>
                    @if ($product->hasMedia($media_gallery_tag))
                    @foreach($product->getMedia($media_gallery_tag) as $media)
                ...
                <div class="col-md-7" id="col-large-image{{ $product->id }}">
                    @if ($product->hasMedia($media_gallery_tag))
                    <div onclick="bigImg('{{ getMediaUrl($product->getMedia($media_gallery_tag)->first()) }}')" style=" margin-bottom: 5px; width: 300px;height: 300px; background-image: url('{{ getMediaUrl($product->getMedia($media_gallery_tag)->first()) }}'); background-size: 300px" id="image{{ $product->id }}">
                    ...
                    <button onclick="cropImage('{{ getMediaUrl($product->getMedia($media_gallery_tag)->first()) }}','{{ $product->id }}')" class="btn btn-secondary">Crop Image
                    </button>
                    <button onclick="crop('{{ getMediaUrl($product->getMedia($media_gallery_tag)->first()) }}','{{ $product->id }}','{{ $gridImage }}')" class="btn btn-secondary">Crop
                    </button>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="translation-table">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @if(!empty($product_translation_history2))
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.get-translate-score-btn', function() {
            var ptId = $(this).data('id');
            if (ptId) {
                $.ajax({
                    url: "/products/product-translation/" +  ptId + "/get-translated-text-score",
                    type: "GET",
                    beforeSend: function() {
                        $("#loading-image").show();
                    }
