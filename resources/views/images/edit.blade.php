...
<div class="row">
  <div class="col-md-6">
    <img src="{{ $image->filename ? (asset('uploads/social-media') . '/' . $image->filename) : ($image->getMedia($media_tags)->first() ? getMediaUrl($image->getMedia($media_tags)->first()) : '') }}" class="img-responsive" alt="">
    ...
    <form class="mt-3" action="{{ route('image.grid.update', $image->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group">
        <strong>Brand:</strong>
        {{ html()->select('brand', $brands, $image->brand)->placeholder('Select a Brand')->class('form-control')->id('product-brand') }}
