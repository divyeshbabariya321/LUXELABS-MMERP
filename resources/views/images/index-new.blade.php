...
<div class="row">
  <div class="col-12 margin-tb mb-3">
    <h2 class="page-heading">New LifeStyle Image Grid</h2>

    <form action="{{ route('image.grid.new') }}" method="GET" class="form-inline align-items-start">
          ...
          @foreach($images as $image)
            ...
            <tr class="expand-{{$image->id}} hidden">
                ...
                <div class="col-md-2 col-xs-3 text-center">
                    <div class="thumbnail">
                        <img src="{{ $image->filename ? (asset('uploads/social-media') . '/' . $image->filename) : ($image->getMedia($media_tags)->first() ? getMediaUrl($image->getMedia($media_tags)->first()) : '') }}" class="img-responsive grid-image" alt="" />
                        <div class="caption">
                            ...
                            @if(auth()->user()->checkPermission('social-create'))
                              ...
                              {{ html()->form('DELETE', route('image.grid.delete', [$image->id]))->style('display:inline')->open() }}
                                <button type="submit" class="btn btn-image"><img src="{{asset('/images/delete.png')}}" /></button>
                              {{ html()->form()->close() }}
                            @endif

                            @if (isset($image->approved_user))
                              <span style="white-space: pre-wrap; ">Approved by {{ $image->approvedUser->name }} on {{ Carbon\Carbon::parse($image->approved_date)->format('d-m') }}</span>
                            @else
                              @if(auth()->user()->checkPermission('social-create'))
                    ...
                    @foreach( $image->productImg( $image->product_id, $image->id ) as $img )
                        @if(!$loop->last)
                            <div class="col-md-2 col-xs-3 text-center">
                                <div class="thumbnail">
                                    <img src="{{ $img->filename ? (asset('uploads/social-media') . '/' . $img->filename) : ($img->getMedia($media_tags)->first() ? getMediaUrl($img->getMedia($media_tags)->first()) : '') }}" class="img-responsive grid-image" alt="" />
                                    ...
                                    <div class="caption">
                                            ...
                                            @if(auth()->user()->checkPermission('social-create'))
                                              ...
                                              {{ html()->form('DELETE', route('image.grid.delete', [$img->id]))->style('display:inline')->open() }}
                                                <button type="submit" class="btn btn-image"><img src="{{asset('/images/delete.png')}}" /></button>
                                              {{ html()->form()->close() }}
                                            @endif

                                            @if (isset($img->approved_user))
                                              <span style="white-space: pre-wrap; ">Approved by {{ \App\Helpers\DevelopmentHelper::approvedUser($img)}} on {{ Carbon\Carbon::parse($img->approved_date)->format('d-m') }}</span>
                                            @else
                                              @if(auth()->user()->checkPermission('social-create'))
                                                {{-- <form action="{{ route('image.grid.approveImage', $image->id) }}" method="POST">
                                                  @csrf
...
<script type="text/javascript">
  ...
  $(document).on('click', '.preview-attached-img-btn', function (e) {     
            ...
            var id = $(this).data('suggestedproductid');
            // $.ajax({
