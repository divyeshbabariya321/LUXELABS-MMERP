              ...
              <div class="col-6 col-md-3">
                ...
                <div class="form-group col-md-3">
                  <select class="form-control change-whatsapp-no" data-customer-id="<?php echo $customer->id; ?>" data-type="whatsapp_number" placeholder="Whatsapp No :">
                      ...
                      @foreach(array_filter($apiInstances) as $number => $apwCate)
                          @if($number != "0")
                ...
                <div class="form-group col-md-3">
                  <select class="form-control update-customer-field select2" data-customer-id="<?php echo $customer->id; ?>" data-type="store_website_id" placeholder="Registration Source">
                      <option value="">-No Selected-</option>
                      @foreach($storeWebsites as $r => $sWeb)
                      ...
                      <div class="row">
                        <div class="col-xs-12">
                          ...
                          <div class="form-group">
                            ...
                            <select name="category_id[]" class="form-control select-multiple" multiple="multiple">
                               <option value="">Category</option>
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ $category->id == $lead->category_id ? "selected" : ''}}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        */ ?>
                        ...
                        <?php $images = $lead->getMedia($mediaTags) ?>
                        @if ($lead->hasMedia($mediaTags))
                          <div class="row">
                            @foreach ($images as $key => $image)
                              ...
                              ">
                              <p>
                                <img src="{{ getMediaUrl($image) ?? '#no-image' }}" class="img-responsive" alt="">
            ...
            @foreach ($customer->orders as $key => $order)
              ...
              <div class="card">
                ...
                <div id="order{{ $key + 1 }}" class="collapse collapse-element" aria-labelledby="headingOrder{{ $key + 1 }}" data-parent="#orderAccordion">
                  ...
                  ?>
                          {{ html()->select('payment_mode', $paymentModes->all(), old('payment_mode') ? old('payment_mode') : $order->payment_mode)->placeholder('Select a mode')->class('form-control') }}
                  ...
                  ?>
                          {{ html()->select('sales_person', $sales_persons, old('sales_person') ? old('sales_person') : $order->sales_person)->placeholder('Select a name')->class('form-control') }}
                                      ...
                                      <tr>
                                          ...
                                          <td>
                                            @if(isset($order_product->product))
                                              ...
                                              @if ($order_product->product->hasMedia($mediaTags))
                                                <img width="150" src="{{ getMediaUrl($order_product->product->getMedia($mediaTags)->first()) }}" data-toggle='tooltip' data-html='true' data-placement='top' title="{{ Auth::user()->hasRole('Admin') || Auth::user()->hasRole('HOD of CRM') ? "<strong>Supplier:</strong> $supplier_initials" : '' }}" />
                                              @endif
                      ...
                      <div class="table-responsive">
                        <table class="table">
                          ...
                          <tbody>
                            <tr>
                              <td>
                                @if ($order->delivery_approval->hasMedia($mediaTags))
                                  @foreach ($order->delivery_approval->getMedia($mediaTags) as $image)
                                    <img width="150" src="{{ getMediaUrl($image) ?? '#no-image' }}" />
                                  @endforeach
                                @endif
                              </td>
                              <td>
                ...
                <tr class="{{ \Carbon\Carbon::parse($view->date)->format('Y-m-d') == date('Y-m-d') ? 'row-highlight' : '' }}">
                  <td>
                    @foreach ($view->products as $product)
                      @if ($product->hasMedia($mediaTags))
                        <img src="{{ getMediaUrl($product->getMedia($mediaTags)->first()) }}" class="img-responsive" style="width: 50px;" alt="">
                      @endif
                    @endforeach
                  </td>
                  <td>{{ \Carbon\Carbon::parse($view->date)->format('d-m') }}</td>
                  <td>
                    @if ($view->hasMedia($mediaTags))
                      @foreach ($view->getMedia($mediaTags) as $image)
      ...
      <ul class="nav nav-tabs" style="flex-grow: 1;justify-content: flex-end">
          <li>
              <form class="d-inline" action="{{ route('instruction.store') }}" method="POST">
                  ...
                  <input type="hidden" name="assigned_to" value="{{ $shortCuts['image_shortcut'] }}">
          ...
          <li>
              <form class="d-inline" action="{{ route('instruction.store') }}" method="POST">
                  ...
                  <input type="hidden" name="assigned_to" value="{{ $shortCuts['price_shortcut'] }}">
          ...
          <li>
              <form class="d-inline" action="{{ route('instruction.store') }}" method="POST">
                  @csrf
                  <input type="hidden" name="customer_id" value="{{ $customer->id }}">
                  <input type="hidden" name="instruction" value="{{ @$users_array[$shortCuts['call_shortcut']] }} call this client">
                  <input type="hidden" name="category_id" value="10">
                  <input type="hidden" name="assigned_to" value="{{ $shortCuts['call_shortcut'] }}">
          ...
          <li>
              <form class="d-inline" action="{{ route('instruction.store') }}" method="POST">
                  ...
                  <input type="hidden" name="assigned_to" value="{{ $shortCuts['screenshot_shortcut'] }}">
          ...
          <li>
              <form class="d-inline" action="{{ route('instruction.store') }}" method="POST">
                  ...
                  <input type="hidden" name="assigned_to" value="{{ $shortCuts['details_shortcut'] }}">
          ...
          <li>
              <form class="d-inline" action="{{ route('instruction.store') }}" method="POST">
                  ...
                  <input type="hidden" name="assigned_to" value="{{ $shortCuts['purchase_shortcut'] }}">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form method="post" id="send-contact-to-user">
            {{ html()->form('POST', url(''))->id('send-contact-user-form')->open() }}
            {{ html()->hidden('customer_id', $customer->id) }}
            {{ html()->select('user_id', $sortUsers, 6)->class('form-control select-user-wha-list multi_select2')->style('width:100%') }}
            {{ html()->form()->close() }}
        </form>
      </div>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form method="post" id="download-contact-to-user">
            {{ html()->form('POST', url(''))->id('download-contact-user-form')->open() }}
            {{ html()->hidden('customer_id', $customer->id) }}
            {{ html()->select('user_id', $sortUsers, 6)->class('form-control select-user-wha-list multi_select2')->style('width:100%') }}
            {{ html()->form()->close() }}
        </form>
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <div class="form-group">
                                <strong>Category</strong>
                                <select name="category_id[]" class="form-control multi_select2 add_lead_category_id" multiple="multiple">
                                  <option value="">Category</option>
                                    @foreach($categories as $category)
        ...
        <div class="modal-content">
            <form action="{{  route('order.store') }}" method="POST" enctype="multipart/form-data" class="add_order_frm" data-reload='1'>
                ...
                <div class="modal-body">
                  ...
                  ?>
                                        {{ html()->select('order_status', $orderStatus->all(), 'Follow up for advance')->placeholder('Select a status')->class('form-control') }}
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                <div class="form-group">
                  ...
                  $paymentModes = new \App\ReadOnly\PaymentModes;
                  ?>
                                        {{ html()->select('payment_mode', $paymentModes->all())->placeholder('Select a mode')->class('form-control') }}
                                </div>
                            </div>
...
@section('scripts')
  ...
  <script type="text/javascript" src="<?php echo config('settings.domain_node_speech_to_text'); ?>/scripts/bundle.js"></script>
  <script type="text/javascript">
      //$(document).ready(function () {
  ...
  $('#create_refund_instruction').on('click', function () {
    ...
    $.ajax({
      ...
      data: {
        ...
        category_id: 1,
        assigned_to: "{{ $shortCuts['image_shortcut'] }}"
      },
      beforeSend: function() {
        $(thiss).text('Loading...');
      }
      ...
      @endif
        ...
        $('.createProduct').on('click', function() {
          ...
          $.ajax({
            ...
            var brands_array = {!! json_encode(\App\Helpers::getUserArray($brands)) !!};
