            ...
            <tr class="{{ \Carbon\Carbon::parse($view->date)->format('Y-m-d') == date('Y-m-d') ? 'row-highlight' : '' }}">
                ...
                <td>
                  @foreach ($view->products as $product)
                    <img src="{{ $product->getMedia($media_tags)->first() ? getMediaUrl($product->getMedia($media_tags)->first()) : '' }}" class="img-responsive" style="width: 50px;" alt="">
                  @endforeach
                </td>
                <td>
                  @if ($view->hasMedia($media_tags))
                    @foreach ($view->getMedia($media_tags) as $image)
                ...
                <td>
                  ...
                  {{ html()->form('DELETE', route('stock.private.viewing.destroy', [$view->id]))->style('display:inline')->open() }}
