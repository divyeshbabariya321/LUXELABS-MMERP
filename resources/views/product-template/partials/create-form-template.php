          ...
          <form method="post" enctype="multipart/form-data" id="product-template-from">
                    ...
                    <select class="form-control template-dropdown-function template_no valid" name="template_no" aria-invalid="false">
                        <?php
                            foreach ($templateArr as $template) {
                                $media = $template->lastMedia($mediaTags);
                                echo '<option value="' . $template->id . '" data-image="' . (($media) ? getMediaUrl($media) : '') . '" data-no-of-images="' . $template->no_of_images . '">' . $template->name . '</option>';
                            }
             ?>
                    </select>
                </div>
           ...
           <div class="default">
                <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                    {{ 
                        html()->text('modifications_array[0][text]')
                           ->class('form-control')
                     }}
                    </div>
                 </div>
                 <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                    {{ 
                        html()->color('modifications_array[0][color]')
                           ->class('form-control')
                     }}
                    </div>
                 </div>
                 <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                    {{ 
                        html()->color('modifications_array[0][background]')
                           ->class('form-control')
                     }}
                    </div>
                 </div>
             ...
             <div class="form-group row">
                ...
                <div class="col-sm-6">
                {{ 
                     html()->select('brand_id', ['' => '-Select-'] + \App\Brand::pluck('name', 'id')->toArray())
                        ->class('form-control')
                  }}
                </div>
             </div>
             ...
             <div class="form-group row">
                ...
                <div class="col-sm-6">
                {{ 
                     html()->select('store_website_id', \App\StoreWebsite::pluck('title', 'id')->toArray())
                        ->class('form-control')
                  }}
                </div>
             </div>
