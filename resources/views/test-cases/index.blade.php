...
@section('content')
            ...
            <div class="row">
                                    ...
                                    <div class="form-group col-md-1 cls_filter_inputbox mr-2 mt-2">
                                        ...
                                        <select class="form-control" name="test_case_status" id="test_case_status">
                                            ...
                                            <option value="<?php echo $testCaseStatus->id; ?>" <?php if ($test_case_status == $testCaseStatus->id) {
                                                echo 'selected';
                                            } ?>><?php echo $testCaseStatus->name; ?></option>
                                            <?php }
                                            ?>
                                        </select>
                                    </div>
                ...
                <div class="col col-md-6">
                    <div class="row">
                        ...
                        <button style="display: inline-block;width: 10%" class="btn btn-sm btn-image btn-add-action openTestCasesModal" data-action="create">
    ...
    @include("test-cases.partials.test-cases-add-edit-modal")
    ...
    <div id="bugtrackingShowFullTextModel" class="modal fade" role="dialog">
        ...
        <script type="text/javascript">
            $(document).ready(function(){
                ...
                // Open modal
                $(document).on('click', '.openTestCasesModal', function(){
                    currentAction = $(this).data('action');
                    if (currentAction === 'create') {
                        modalTitle.text('Add Test Cases');
                        name.removeClass('name');
                        suite.removeClass('suite');
                        module_id.removeClass('module_id');
                        precondition.removeClass('precondition');
                        assign_to.removeClass('assign_to_edit');
                        step_to_reproduce.removeClass('step_to_reproduce');
                        expected_result.removeClass('expected_result');
                        test_status_id.removeClass('test_status_id');
                        website.removeClass('website');
                        form.trigger('reset');

                        form.find('input[name=id]').remove();
                        form.attr('action', '{{ route("test-cases.store") }}');
                    } else if (currentAction === 'edit') {
                        $('.edit-h3').text('Edit Test Case');
                        name.addClass('name');
                        suite.addClass('suite');
                        module_id.addClass('module_id');
                        precondition.addClass('precondition');
                        assign_to.addClass('assign_to_edit');
                        step_to_reproduce.addClass('step_to_reproduce');
                        expected_result.addClass('expected_result');
                        test_status_id.addClass('test_status_id');
                        website.addClass('website');

                        var testCaseId = $(this).data('id');
                        form.append('<input class="form-control id" name="id" type="hidden" value="' + testCaseId + '">');
                        
                        form.attr('action', '{{ route("test-cases.update") }}');
                    }
                    modal.show();
                });
