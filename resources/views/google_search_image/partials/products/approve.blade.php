    ...
    <div class="productGrid" id="productGrid">
        ...
        <tbody>
                ...
                <td>
                    <div class="row">
                        @php
                        $images = $product->getMedia($google_text_search)->all();
                        @endphp
                        @if(count($images) != 0)
                        @foreach($images as $image)
                            <div class="col-md-2 col-half-offset"><div class="text-center"><img height="100" width="100" src="{{ getMediaUrl($image) }}" onclick="openImage(this.src)"><br><input type="checkbox" class="checkbox{{ $product->id }}" value="{{ $image->id }}"></div></div>
                        @endforeach
