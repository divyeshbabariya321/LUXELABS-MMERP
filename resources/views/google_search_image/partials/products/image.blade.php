            ...
            <div class="row">
                @foreach ($products as $product)
                    <div class="col-md-3 col-xs-6 text-left" style="border: 1px solid #cccccc;">
                        <?php if($product->hasMedia($media_tags)) { ?>
                            <a href="{{ route('products.show', $product->id) }}" target="_blank"><img src="{{ $product->getMedia($media_tags)->first() ? getMediaUrl($product->getMedia($media_tags)->first()) : '' }}" class="img-responsive grid-image" alt=""/></a>
                        <?php } ?>
                        ...
                        <?php if($product->hasMedia($media_tags)) { ?>
                            <input type="hidden" id="img{{ $product->id }}" value="{{ getMediaUrl($product->getMedia($media_tags)->first()) }}">
