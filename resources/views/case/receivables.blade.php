    ...
    <div class="row">
      <div class="col-lg-12 margin-tb">
        <h2 class="page-heading">Receivable History - <a href="{{route('case.show',$case->id)}}" title="Case Details">{{ $case->case_number }} </a>(Lawyer :<a
                        href="{{route('lawyer.show',$case->lawyer_id)}}" target="_blank" title="View Lawyer Detail"> {{ $case->lawyer?->name }})</a></h2>
        <div class="pull-left">
            ...
            <tr>
              ...
              <td>
                <div class="d-flex">
                  ...
                  {{ html()->form('DELETE', route('case.receivable.destroy', [$case->id, $receivable->id]))->style('display:inline')->open() }}
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-12 col-lg-12 @if($errors->has('receivable_date')) has-danger @elseif(count($errors->all())>0) has-success @endif">
                                <div class="form-group">
                                    {{ html()->label('Receivable date', 'receivable_date')->class('form-control-label') }}
                                    {{ html()->input('date', 'receivable_date')->class('form-control ' . ($errors->has('receivable_date') ? 'form-control-danger' : (count($errors->all()) > 0 ? ' form-control-success' : '')))->required() }}
                            ...
                            <div class="col-md-12 col-lg-12 @if($errors->has('receivable_amount')) has-danger @elseif(count($errors->all())>0) has-success @endif">
                                <div class="form-group">
                                    {{ html()->label('Payable Amount', 'receivable_amount')->class('form-control-label') }}
                                    {{ html()->number('receivable_amount')->class('form-control ' . ($errors->has('receivable_amount') ? 'form-control-danger' : (count($errors->all()) > 0 ? ' form-control-success' : '')))->attribute('required', ) }}
                            ...
                            <div class="col-md-12 col-lg-12 @if($errors->has('received_date')) has-danger @elseif(count($errors->all())>0) has-success @endif">
                                <div class="form-group">
                                    {{ html()->label('Received Date', 'received_date')->class('form-control-label') }}
                                    {{ html()->input('date', 'received_date')->class('form-control ' . ($errors->has('received_date') ? 'form-control-danger' : (count($errors->all()) > 0 ? ' form-control-success' : ''))) }}
                            ...
                            <div class="col-md-12 col-lg-12 @if($errors->has('received_amount')) has-danger @elseif(count($errors->all())>0) has-success @endif">
                                <div class="form-group">
                                    {{ html()->label('Received Amount', 'received_amount')->class('form-control-label') }}
                                    {{ html()->number('received_amount')->class('form-control ' . ($errors->has('received_amount') ? 'form-control-danger' : (count($errors->all()) > 0 ? ' form-control-success' : ''))) }}
