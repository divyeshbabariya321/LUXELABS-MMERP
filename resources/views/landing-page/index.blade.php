...
<div class="row" id="common-page-layout">
	...
	<br>
	@include('partials.flash_messages', ['colAlert' => true])
		<div class="col-lg-12 margin-tb">
		<div class="row">
					...
					<form class="form-inline message-search-handler mb-2 fr" method="post" style="float: right;">
						<div class="row">
							<div class="form-group" style="margin-left: 2px">
								...
								{{ html()->text("keyword", request("keyword"))->class("form-control")->placeholder("Enter Product d") }}
							</div>
							<div class="form-group" style="margin-left: 2px">
								<label for="stock_status">Stock Status:</label>
								{{ html()->select("stock_status", [0 => "Sold out", 1 => "In Stock"], request("stock_status"))->class("form-control")->placeholder("Stock status") }}
							</div>
							<div class="form-group" style="margin-left: 2px">
								<label for="status">Status:</label>
								{{ html()->select("status", array_values($statuses), request("statuses"))->class("form-control")->placeholder("Status") }}
<!--								--><?php //echo Form::select("status",[0 => "De-Active", 1 => "Active"],request("status"),["class"=> "form-control","placeholder" => "Status"])?>
							</div>
							<div class="form-group" style="margin-left: 2px">
								<label for="status">Product Status:</label>
								{{ html()->select("product_status", [9 => "Final Approval", 4 => "Auto Crop"], request("product_status"))->class("form-control")->placeholder("Product Status") }}
							</div>
							<div class="form-group" style="margin-left: 2px">
								<label for="status">Brand:</label>
								{{ html()->text("brand", request("brand"))->class("form-control")->placeholder("Brand") }}
							</div>
							<div class="form-group" style="margin-left: 2px">
								<label for="">Date:</label>
								{{ html()->input('date', "date_from", request("date_from"))->class("form-control")->attribute('pl
								aceholder', "From date") }}
								{{ html()->input('date', "date_to", request("date_to"))->class("form-control")->placeholder("To date") }}
							</div>
...
<script type="text/javascript" src="{{ asset('/js/landing-page.js') }} "></script>
