          ...
          @php $count = 0; @endphp
              @foreach ($order->order_product as $order_product)
                @if ($order_product->product)
                  @if ($order_product->product->hasMedia($media_tags))
                     <img alt="Image" style="width: 50px; height: 50px;" src="{{ $order_product->product->getMedia($media_tags)->first()->getAbsolutePath() }}">
                        @php $count++; @endphp
                  @endif
                @endif
              @endforeach
