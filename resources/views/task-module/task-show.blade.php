...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left d-flex">
                <h3>{{ $task->is_statutory == 3 ? 'Discussion' : ($task->is_statutory == 1 ? 'Statutory' : '') }} Task
                    Page</h3>

                @if ($task->users->contains(Auth::id()) || $task->assign_from == Auth::id())
                    @if ($task->is_completed == '')
                        <button type="button" class="btn btn-image task-complete mt-3" data-id="{{ $task->id }}"><img
                                    src="/images/incomplete.png" /></button>
                    @else
                        @if ($task->assign_from == Auth::id())
                            <button type="button" class="btn btn-image task-complete mt-3" data-id="{{ $task->id }}">
                                <img src="/images/completed-green.png" /></button>
                        @else
                            <button type="button" class="btn btn-image mt-3"><img src="/images/completed-green.png" />
                            </button>
                        @endif
                    @endif
                @endif
                ...
                @if ($task->is_watched == 1)
                    <button type="button" class="btn btn-image make-watched-task mt-3" data-taskid="{{ $task->id }}">
                        <img src="/images/starred.png" /></button>
                @else
                    <button type="button" class="btn btn-image make-watched-task mt-3" data-taskid="{{ $task->id }}">
                        <img src="/images/unstarred.png" /></button>
                @endif

                @if ($task->is_flagged == 1)
                    <button type="button" class="btn btn-image flag-task mt-3" data-id="{{ $task->id }}"><img
                                src="/images/flagged.png" /></button>
                @else
                    <button type="button" class="btn btn-image flag-task mt-3" data-id="{{ $task->id }}"><img
                                src="/images/unflagged.png" /></button>
                @endif
                @if ($task->task_subject)
                    <h4 style="margin-top: 23px !important;">Subject : {{ $task->task_subject }}</h4>
                @endif
                <span type="text" name="subject" id="task_subject_field" class="form-control span-sm hidden"
            ...
            <div class="pull-right mt-4">
                @if (count($hiddenRemarks))
                    <button type="button" class="btn btn-default" data-toggle="modal"
                            data-target="#hidden_remark_modal">Hidden Notes
                    </button>
                @endif                
            </div>
        </div>
    ...
    <div class="row">
        @if($task->is_statutory == 3)
            <div class="col-md-12">
                <form class="form-inline message-search-handler mb-3" method="get"
                      action="{{ route('task.module.show', $task->id) }}">
                ...
                <div class="infinite-scroll">
                    <table class="table table-striped table-bordered">
                        ...
                        <tr>
                            <td>
                                <input type="text" id="create-note-field-for-appointment" class="form-control input-sm"
                                       name="note" placeholder="Add New Update..." value="">
                            </td>
                            <td></td>
                            <td></td>
                        </tr>
                        @foreach ($taskNotes as $key=>$note)
                            <tr>
                                <td>
                                    {{ $note->remark }}
                                    <button type="button" class="btn btn-image create-quick-task-button"
                                            data-remark="{{ $note->remark }}" data-id="{{ $note->id }}"
                                            style="padding-top: 0px !important;"><img src="/images/add.png"
                                                                                      style="width: 12px !important;" />
                                    </button>
                                    <div style="display:none;" id="div{{ $note->id }}">
                                        <select class="form-control selectpicker user-list" data-live-search="true"
                                                style="display:none !important;" data-remark="{{ $note->remark }}"
                                                data-remark-id="{{ $note->id }}">
                                ...
                                @if(isset($note->singleSubnotes))
                                    <input type="hidden" id="remark-text{{ $note->id }}"
                                           value="{{ $note->singleSubnotes->remark }}">
                                    <input type="hidden" id="remark-id{{ $note->id }}"
                                           value="{{ $note->singleSubnotes->id }}">
                                        ...
                                        <div class="col-md-12">
                                            <div class="col-md-3">
                                                <table class="table table-style">
                                                    <tbody>
                                                    <tr class="tr-style">
                                                        <th class="table-head-row expand-row table-hover-cell"
                                                            id="remark{{$note->id}}">
                                            ...
                                            <div class="col-md-9">
                                                <div class="row"
                                                    ...
                                                    <div class="col-md-8" style="padding:5px;">
                                                        <span class="table-head-row">   <input type="text"
                                                                                               class="form-control input-sm create-subnote-for-appointment"
                                                                                               data-id="{{ $note->id }}"
                                                                                               name="note"
                                                                                               placeholder="Note"
                                                                                               value=""> </span>
                                                    ...
                                                    <div class="col-md-4" style="padding:5px;">
                                                        <span style="vertical-align: middle !important;"
                                                              class="table-head-row th-add-user th-created_at"
                                                              id="created{{$note->id}}"> {{ $note->singleSubnotes->created_at->format('d-m-Y H:i:s') }}   </span>
                                                        <input type="hidden" id="current-remark-id">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-image create-quick-task-note-button"
                                                onclick="createTaskNoteButton({{  $note->id }})" title="Add Task Note">
                                            <img src="/images/add.png" /></button>

                                        <div style="display:none;" id="divremark{{ $note->id }}">
                                            <select class="form-control selectpicker" data-live-search="true"
                                                    style="display:none !important;"
                                                    onchange="sendUserTask({{ $note->id }})"
                                                    id="user-selected{{ $note->id }}">
                                                @foreach($users as $user)
                                                    <option value="{{ $user->id }}">{{ $user->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <button type="button" class="btn btn-image" data-toggle="modal"
                                                data-target="#chat-list-history{{ $note->id }}" title="Chat History">
                                            <img src="/images/chat.png" /></button>
                                        <button type="button" class="btn btn-image"
                                                onclick="archiveRemark({{ $note->singleSubnotes->id }} , {{ $note->id }})"
                                                title="Archive Remark"><img src="/images/archive.png" /></button>

                                        <button type="button" class="btn btn-image" data-toggle="modal"
                                                data-target="#archive-list-history{{ $note->id }}"
                                                title="Archive Remark History"><img src="/images/advance-link.png" />
                                        </button>
                                        <button type="button" class="btn remove-task-note"
                                                data-task-note-id="{{ $note->id }}" title="Delete"><i
                                                    class="fa fa-trash" aria-hidden="true"></i></button>
                                        <button type="button" class="btn hide-task-note"
                                                data-task-note-id="{{ $note->id }}" title="Hide"><i
                                                    class="fa fa-eye-slash" aria-hidden="true"></i></button>
                                        @if ($note->is_flagged == 1)
                                            <button type="button" class="btn btn-image flag-task pd-5"
                                                    data-id="{{ $note->id }}"><img
                                ...
                                @else
                                    ...
                                    <td class="td-style">
                                        ...
                                        <div class="row" style="margin-bottom:0px;margin-left:0px;margin-right:0px;">
                                            ...
                                            <div class="col-md-4" style="padding:5px;">
                                                <span style="vertical-align: middle !important;"
                                                      class="table-head-row th-add-user th-created_at"
                                                      id="created{{$note->id}}">   </span> <input type="hidden"
                                                                                                  id="current-remark-id">
                                            </div>
                                        </div>
                                    ...
                                    <td>
                                        ...
                                        <button type="button" class="btn btn-image" data-toggle="modal"
                                                data-target="#chat-list-history{{ $note->id }}" title="Chat History">
                                            <img src="/images/chat.png" /></button>
                                        <button type="button" class="btn btn-image" onclick="archiveRemarkRefresh()">
                                            <img src="/images/archive.png" /></button>
        ...
        @else
            <table class="table table-striped table-bordered">
                ...
                <tr>
                    <th><input type="text" class="form-control input-sm create-subnote-for-appointment" data-id=""
                               name="note" placeholder="Note" value=""></th>
                    <th></th>
                    <th></th>
                </tr>
                </thead>
                ...
                @foreach($chatMessages as $chatMessage)
                    <tr>
                        ...
                        <td></td>
                        <td><a title="Dialog" href="javascript:;" class="btn btn-xs btn-secondary ml-1 create-dialog"><i
                                        class="fa fa-plus" aria-hidden="true"></i></a>
                            <button type="button" class="btn btn-xs btn-image load-communication-modal"
                                    data-is_admin="{{ Auth::user()->hasRole('Admin') }}"
                                    data-is_hod_crm="{{ Auth::user()->hasRole('HOD of CRM') }}" id="message-wrappers"
                                    data-message="{{$chatMessage->message}}" data-object="task" data-attached="1"
                                    data-object="task" data-id="{{ $chatMessage->task_id}}" title="Load messages"><img
                                        src="https://erp.theluxuryunlimited.com/images/chat.png" alt=""
                                        style="cursor: default;"></button>
                            <button type="button" class="btn preview-img-btn pd-5" data-id="{{ $chatMessage->id}}"><i
                                        class="fa fa-list" aria-hidden="true"></i></button>
                            <i class="fa fa-mail" aria-hidden="true"></i> <a title="Remove" href="javascript:;"
                                                                             class="btn btn-xs btn-secondary ml-1 delete-message remove-task-note"
                                                                             data-id="{{ $chatMessage->id}}"><i
                                        class="fa fa-trash" aria-hidden="true"></i></a> <i class="fa fa-eye"
