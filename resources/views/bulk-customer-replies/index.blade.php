            ...
            <div class="panel-body">
                ...
                <div>
                    ...
                    @foreach($keywords as $keyword)
                        <a href="{{ action([\App\Http\Controllers\BulkCustomerRepliesController::class, 'index'], ['keyword_filter' => $keyword->value]) }}" style="font-size: 14px; display:inline-block;" class="label label-default bg-light text-dark mb-2">{{$keyword->value}}</a>
                    @endforeach
                </div>
                <div class="mt-2">
                    <strong>Auto Generated</strong><br>
                    @foreach($autoKeywords as $keyword)
                        <a href="{{ action([\App\Http\Controllers\BulkCustomerRepliesController::class, 'index'], ['keyword_filter' => $keyword->value]) }}" style="font-size: 14px; display:inline-block;" class="label label-default bg-light text-dark mb-2">{{$keyword->value}}({{$keyword->count}})</a>
                    @endforeach
                </div>
            </div>
        </div>
                            ...
                            <tr data-customer_id="{{ $customer->id }}" class="customer-id-remove-class">
                                ...
                                <td class="p-0 pl-1 pt-2">
                                    ...
                                    <select class="form-control change-whatsapp-no-bulk" data-customer-id="<?php echo $customer->id; ?>" data-type="whatsapp_number">
                                        ...
                                        @foreach(array_filter($whatsappConfigs) as $number => $apwCate)
                                            @if($number != "0")
            ...
            <form action="{{ route('bulk-messages.whatsapp-no') }}" method="post">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                       @csrf
                        {{ html()->select("whatsapp_no", $whatsappNos)->class("form-control select2 whatsapp_no")->style("width:100%") }}
                    </div>
                    <div class="modal-footer">
...
@section('scripts')
    <!-- <script type="text/javascript" src="{{ mix('webpack-dist/js/site-helper.js') }} "></script> -->
