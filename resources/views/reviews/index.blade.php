              ...
              @foreach ($accounts as $account)
                <tr class="{{ $account->has_posted_reviews() ? 'text-danger' : '' }}">
                  ...
                  <td>
                    ...
                    {{ html()->form('DELETE', route('account.destroy', [$account->id]))->style('display:inline')->open() }}
              ...
              @foreach ($posted_reviews as $review)
                <tr>
                  ...
                  <td>
                    ...
                    {{ html()->form('DELETE', route('review.destroy', [$review->id]))->style('display:inline')->open() }}
                      <button type="submit" class="btn btn-image"><img src="/images/delete.png" /></button>
                    {{ html()->form()->close() }}
                  </td>
                </tr>
              ...
              @foreach ($complaints as $complaint)
                <tr>
                  ...
                  <td>
                    ...
                    @if ($complaint->hasMedia($media_tags))
                      <ul>
                        @foreach ($complaint->getMedia($media_tags) as $image)
                          <li><a href="{{ getMediaUrl($image) }}" target="_blank"><img src="{{ getMediaUrl($image) }}" class="img-responsive" /></a></li>
                        @endforeach
                      </ul>
                    @endif
                  </td>
                  ...
                  <td>
                    ...
                    {{ html()->form('DELETE', route('thread.destroy', [$complaint->id]))->style('display:inline')->open() }}
                      <button type="submit" class="btn btn-image"><img src="/images/delete.png" /></button>
                    {{ html()->form()->close() }}
            ...
            @foreach ($instagram_dm_reviews as $schedule)
              <tr>
                ...
                <td>                  
                  ...
                  {{ html()->form('DELETE', route('review.schedule.destroy', [$schedule->id]))->style('display:inline')->open() }}
                  <button type="submit" class="btn btn-image"><img src="/images/delete.png" /></button>
                  {{ html()->form()->close() }}
                </td>
              </tr>
...
@section('scripts')
  ...
  <script type="text/javascript">
    ...
    var accounts_array = @json($accounts_array);
