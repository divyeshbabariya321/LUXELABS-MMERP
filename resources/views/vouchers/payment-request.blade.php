...
@endsection
        ...
        {{ html()->form('POST', route('voucher.payment.request-submit'))->id('payment-request-form')->open() }}
...
<script>
    $(document).on('submit', '#payment-request-form', function (e) {
        e.preventDefault();
        $.ajax({
            url: "{{ route('voucher.payment.request-submit') }}",
            method: "POST",
            data: $(this).serialize(),
