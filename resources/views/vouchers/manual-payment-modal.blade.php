    ...
    <form action="{{route('voucher.payment.manual-payment-submit')}}" method="post" id="payment-submit-form">
    @csrf
...
<script>
$(document).on('submit', '#payment-submit-form', function (e) {
    e.preventDefault();
    $.ajax({
        url: "{{route('voucher.payment.manual-payment-submit')}}",
        method: "POST",
        data: $(this).serialize(),
