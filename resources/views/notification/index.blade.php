                ...
                <form action="{{ route('notifications') }}" method="GET">
                    <div class="row">
                        ...
                        <div class="col-md-6">
					        ...
					        ?>
                            {{ html()->select('sort_by', $sort_by, isset($sort) ? $sort : '')->placeholder('Select a option')->class('form-control') }}
                        </div>
                        <div class="col-md-3">
    ...
    <table class="table notification-table">
        @foreach ($notifications as $notification)
            <tr class="{{ $notification->isread ? 'isread' : '' }}">
                <td>
                    <a class="notification-link" href="{{ $notification->sale_id ? route('sales-item.show',$notification->sale_id) : route('products.show',$notification->product_id)  }}">
