                    ...
                    <tbody>
                        @foreach ($orders_array as $key => $order)
                            ...
                            if($order->supplier_discount_info_id) {
                                $supplier = getSupplierDiscountInfo($order->supplier_discount_info_id);
                            } else {
                                $supplier = getProductSupplier($order->product_id);
                            }
                            @endphp

                            @php
                                $status_color = getOrderPurchaseProductStatus($order->purchase_product_status_id);
                            ...
                            @if(!empty($dynamicColumnsToShowPp))
                                ...
                                <tr class="{{ \App\Helpers::statusClass($order->assign_status ) }}" style="background-color: {{$status_color->status_color ?? ""}}!important;">
                                    ...
                                    <td>
                                        ...
                                        @php
                                            $order_product = getOrderProduct($order->order_product_id);
                                        @endphp
                                    </td>
                                    @endif
                                            ...
                                            <div class="">
                                                ...
                                                @if ($order_product && $order_product->product)
                                                    ...
                                                    @if ($order_product->product->hasMedia($mediaTags))
                                                        <span class="td-mini-container">
                                                            <br/>
                                                            <a style="color:#000!important;" data-fancybox="gallery" href="{{ getMediaUrl($order_product->product->getMedia($mediaTags)->first()) }}">View</a>
                                                        </span>
                                                    @endif
                                                @endif
                                            </div>
                                                ...
                                                foreach($order->orderProducts as $pid){
                                                  ...
                                                  if($product){
                                                    if ($product->hasMedia($mediaTags)){
                                                        $productImages = $product->getMedia($mediaTags);
