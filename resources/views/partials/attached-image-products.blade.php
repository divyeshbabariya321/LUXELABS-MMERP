...
@if(count($suggestedProductsLists))
    @foreach ($suggestedProductsLists as $key => $list)
        @if ($list->getMedia)
            <div class="row">
                <div class="col-md-12">
                    <br>
                        ...
                        <div class="row pl-4 pr-4" style="padding: 0px; margin-bottom: 8px;">
                            <div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input select-pr-list-chk"
                                    checked="checked" id="defaultUnchecked_{{ $product->id . '0' . $customer_id }}">
                                <label class="custom-control-label"
                                    for="defaultUnchecked_{{ $product->id . '0' . $customer_id }}"></label>
                            </div>
...
@foreach ($productsLists as $list)
    @if (count($list->products) > 0)
        ...
        @foreach ($list->products as $kr => $pr)
            ...
            @endphp
            @if ($product->hasMedia($attach_image_tag))
                    @php
