            ...
            <tr>
              ...
              <td>
                @if ($product->hasMedia($media_tags))
                  @foreach ($product->getMedia($media_tags) as $image)
                    <img src="{{ getMediaUrl($image) }}" class="img-responsive m-1" width="50px" alt="">
                  @endforeach
              ...
              <td>
                ...
                {{ html()->form('DELETE', route('vendors.product.destroy', [$product->id]))->style('display:inline')->open() }}
