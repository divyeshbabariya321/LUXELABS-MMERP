    ...
    <form action="{{route('filteredNewDevTaskPlanner')}}" method="get" class="form-inline">
        <div class="col-md-4 col-lg-2 col-xl-2">
            <div class="form-group">
                {{ html()->text('search_term', !empty(request()->search_term) ? request()->search_term : null)->class('form-control')->placeholder('Search Term') }}
            </div>
        </div>
        <div class="col-md-4 col-lg-2 col-xl-2">
            <div class="form-group">
                {{ html()->select('module', !empty($modules) ? $modules : array(), !empty(request()->module) ? request()->module : null)->class('form-control')->placeholder('Select a module') }}
            </div>
        </div>
        <div class="col-md-4 col-lg-2 col-xl-2">
            {{ html()->select('user', !empty($users) ? $users : array(), !empty(request()->user) ? request()->user : null)->class('form-control')->placeholder('Select A User') }}
        </div>
        <div class="col-md-4 col-lg-2 col-xl-2">
            <div class="form-group">
                {{ html()->select('status', !empty($statuses) ? $statuses : array(), !empty(request()->status) ? request()->status : null)->class('form-control')->placeholder('Status') }}
            </div>
        </div>
        <div class="col-md-4 col-lg-2 col-xl-2">
