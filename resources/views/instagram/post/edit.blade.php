            ...
            <div class="card media-manager">
                <div class="card-header pl-3">
                    ...
                    <div class="card-options">
                        <form method="post" action="{{ route('post.destroy', $post) }}" onsubmit="return confirm('{{ __('Confirm delete?') }}');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm">
                                <i class="fe fe-trash"></i> {{ __('Delete post') }}
                            </button>
                        </form>
                    </div>
                </div>
                <div class="pl-3 pr-3 pb-3">
                    ...
                    @else
                        <div class="alert alert-danger">
                            <i class="fe fe-alert-triangle mr-2"></i> {{ __('No media files found') }}
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="p-3">
                        ...
                        <div class="form-group">
                            ...
                            <textarea rows="3" name="caption" class="form-control caption-text" placeholder="{{ __('Compose a post caption') }}" data-emojiable="true">{{ $post->caption }}</textarea>
                        </div>
                        @endif
                        ...
                        <div class="form-group">
                            <label class="custom-control custom-checkbox">
                                ...
                                <span class="custom-control-label">{{ __('Schedule') }}</span>
                            </label>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="input-icon">
                                ...
                                <input type="text" name="scheduled_at" value="{{ $post->scheduled_at }}" class="form-control dm-date-time-picker scheduled-at" placeholder="{{ __('Schedule at') }}">
                            </div>
                        </div>
                    ...
                    <div class="card-footer p-3">
                        <button type="submit" class="btn btn-primary btn-block btn-schedule d-none">
                            <i class="fe fe-clock"></i> {{ __('Schedule post') }}
                        </button>
                        <button type="submit" class="btn btn-success btn-block btn-publish mt-0">
                            <i class="fe fe-check"></i> {{ __('Publish now') }}
                        </button>
                    </div>
                </div>
            </form>
