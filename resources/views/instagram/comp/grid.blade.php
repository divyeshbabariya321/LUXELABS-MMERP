...
@section('large_content')
    <div class="row">
        ...
        <div class="col-md-12">
            <h1 class="text-center" style="background: #CCCCCC;padding: 20px">Competitor Instagram Page</h1>
        </div>
        <div class="col-md-12">
            <div id="exTab2" class="container">
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#one" data-toggle="tab" class="btn btn-image">Following & Followers</a>
                    </li>
                    <li>
                        <a href="#two" data-toggle="tab" class="btn btn-image">Commentors</a>
                    </li>
                </ul>
            </div>
                                ...
                                <table class="table table-striped">
                                    ...
                                    @foreach($item['medias'] as $media)
                                        <tr>
                                            ...
                                            <td>
                                                <img src="{{ $media['display_url']  }}" alt="Image" style="width: 120px;">
                                            </td>
                                            <td>
                                                {{ $media['comments']['caption']['text'] ?? 'N/A' }}
                                            </td>
                                            <td style="width: 600px;">
                                                <table class="table table-striped">
                                                    @foreach($media['comments']['comments'] as $cmt)
                                                        <tr>
                                                            <td>
                                                                <a class="show-overview" data-username="{{$cmt['user']['username']}}" data-uid="{{$cmt['user']['pk']}}">{{$cmt['user']['username']}}</a>
                                                            </td>
                                                            <td>
                                                                {{ $cmt['text'] }}
                                                            </td>
                                            ...
                                            <td>
                                                @if(!\App\ColdLeads::where('username', $cmt['user']['username'])->first() && !\App\Customer::where('instahandler', $cmt['user']['username'])->first())
                                                    <button  class="btn btn-info btn-sm add add-to-cold-leads username-{{$cmt['user']['username']}}" data-type="cold" data-username="{{$cmt['user']['username']}}" data-id="{{$cmt['user']['pk']}}" data-name="{{$cmt['user']['name'] ?? $cmt['user']['username']}}" data-bio="" data-imageurl="" class="btn btn-sm btn-info" title="Add To Cold Leads">
                                                        <i class="fa fa-plus"></i>
                                                    </button>
                                                @endif
                                                @if(!\App\Customer::where('instahandler', $cmt['user']['username'])->first())
                                                    <button class="btn btn-sm btn-success add add-to-customers username-{{$cmt['user']['username']}}" data-type="customer" data-username="{{$cmt['user']['username']}}" data-id="{{$cmt['user']['pk']}}" data-name="{{$cmt['user']['name'] ?? $cmt['user']['username']}}" data-bio="" data-imageurl="" class="btn btn-sm btn-success" title="Add to Customers">
                                                        <i class="fa fa-plus"></i>
                                                    </button>
                                                @endif
                                            </td>
