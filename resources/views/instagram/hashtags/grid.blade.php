...
@section('large_content')
   ...
   <div class="row">
       <div class="col-md-12 p-0">
            <h2 class="page-heading">HASH TAG MONITORING AND COMMENTING  - MISC ACCOUNTS: @if(isset($hashtag)) #{{ $hashtag->hashtag }} ({{ count($medias) }} Posts) @if(config('settings.instagram_main_account') == true) <spam style="color: red;"> ADMIN ACCOUNT PLEASE COMMENT CAREFULLY</spam> @endif  @endif</h2>
       </div>
   </div>
    <div class="row">
        <div class="col-md-12">
        ...
        <div class="col-md-2">
            <p>
                <a href="{{ action([\App\Http\Controllers\HashtagController::class, 'index']) }}" style="color:#343a40;">Show All Targeted Hashtags</a>
            </p>
        </div>
...
// "comment"
    ...
    function posttoInstagram(){
        ...
        $.ajax({
            url: '{{action([\App\Http\Controllers\InstagramPostsController::class, 'createPost'])}}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                'type': 'post',
