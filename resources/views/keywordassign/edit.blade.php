...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-6">
        <div class="form-group">
            <strong>Name:</strong>
            <input type="text" name="keyword" required="" class="form-control" id="keyword" value="{{$keywordassign[0]->keyword}}" />
        </div>
        <div class="form-group">
            ...
            <select name="task_category" required="" class="form-control" id="task_category">
                ...
                @foreach($task_category as $category_row)
                    <option value="{{$category_row->id}}"  @selected($category_row->id == $keywordassign[0]->task_category)>
                        {{$category_row->title}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" required="" name="task_description" id="task_description">{{$keywordassign[0]->task_description}}</textarea>
        </div>
        <div class="form-group">
            ...
            <select name="assign_to" required="" class="form-control" id="assign_to">
                ...
                @foreach($userslist as $row_users)
                    <option value="{{$row_users->id}}"  @selected($row_users->id == $keywordassign[0]->assign_to)>
                        {{$row_users->name}}
                    </option>
                @endforeach
            </select>
