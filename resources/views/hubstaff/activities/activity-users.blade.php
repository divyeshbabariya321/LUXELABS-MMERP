                            ...
                            <td colspan="6">
                                ...
                                <?php if (! empty($user['tasks'])) { ?>
                                    <?php foreach ($user['tasks'] as $ut) { ?>
                                        <?php
                                        @[$taskid, $devtask, $taskName, $estimation, $status, $devTaskId] = explode('||', $ut);
                                        $trackedTime = \App\Helpers\DevelopmentHelper::trackedTimeHubstaffActivity($taskid);
                                        $time_history = \App\Helpers\DevelopmentHelper::timeHistoryDeveloperTaskHistory($devTaskId);
                                        if ($time_history) {
            ...
            <div class="modal-body">
                ...
                @if(auth()->user()->isAdmin())
                    <div class="form-group">
                        ...
                        {{ html()->select("fetch_user_id", $usersForDropdownView, request('fetch_user_id'))->class("form-control select2")->style("width:100%") }}
                    </div>
                @endif
            </div>
            <div class="modal-footer">
