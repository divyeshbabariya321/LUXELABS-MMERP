          ...
          ?>
                ...
                @foreach (array_unique($listOFtask) as $li => $l)
                  @if(!empty($l)) 
                      <?php
            [$type, $id] = explode('-', $l);
          $task = \App\Helpers\DevelopmentHelper::getDeveloperTask($id);
          $estMin = $task->estimate_minutes ?? 0;

          $time_history = \App\Helpers\DevelopmentHelper::getDeveloperTaskHistory($id);
          ?>
                      @if($li==0)
          ...
          ?>
            ...
            <span style="word-break:break-all;" class="show-full-taskStatus-{{$k.$rndNum}} hidden">
              ...
              <div class="form-group" style="margin:0px;">
                   @if(isset($member))
                   <?php
                    $eficiency = \App\Helpers\DevelopmentHelper::getEfficiency($member->user_id, $record->OnDate, $record->onHour);
          $user_input = null;
          $admin_input = null;
          if ($eficiency) {
