		      ...
		      <div class="modal-body">
				  ...
				  <div class="form-row">
					  <div class="form-group col-md-6">
					    ...
					    {{ html()->multiselect("brand_id[]", $brandName)->class("form-control select-2")->style("width:100%;") }}
					  </div>
					  <div class="form-group col-md-6">
					    <label for="category_id">Category</label>
					    {{ html()->multiselect("category_id[]", $categoryTitle)->class("form-control select-2")->style("width:100%;") }}
					  </div>
				  </div>
				  <div class="form-group">
				  ...
				  <div class="form-row">
				  	...
				  	<div class="form-row">
					    <div class="form-group col-md-6">
					      ...
					      {{ html()->select("minute", config("constants.cron_minutes"))->class("form-control")->id("cron_minutes") }}
					    </div>
					    <div class="form-group col-md-6">
					      <label for="cron_hours">Hour(s)</label>
					      {{ html()->select("hour", config("constants.cron_hours"))->class("form-control")->id("cron_hours") }}
					    </div>
					    <div class="form-group col-md-6">
					      <label for="cron_months">Months(s)</label>
					      {{ html()->select("month", config("constants.cron_months"))->class("form-control")->id("cron_months") }}
					    </div>
					    <div class="form-group col-md-6">
					      <label for="cron_days">Day(s)</label>
					      {{ html()->select("day_of_month", config("constants.cron_days"))->class("form-control")->id("cron_days") }}
					    </div>
					    <div class="form-group col-md-6">
					      <label for="cron_weekdays">Weekday(s)</label>
					      {{ html()->select("day_of_week", config("constants.cron_weekdays"))->class("form-control")->id("cron_weekdays") }}
					    </div>
					</div>
		      </div>
...
@section('scripts')
	...
	<script type="text/javascript" src="{{ mix('webpack-dist/js/clndr/clndr.min.js') }} "></script>
	 ...
	 // Assuming you've got the appropriate language files,
	    ...
	    // Here's some magic to make sure the dates are happening this month.
	     var currentMonth = moment().format('YYYY-MM');
 		 ...
 		 var eventArray = JSON.parse('@json($listEvents)');
