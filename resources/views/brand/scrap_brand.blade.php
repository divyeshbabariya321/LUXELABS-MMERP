    ...
    <form class="" action="/scrap/scrap-brand">
        <div class="row">
            <div class="form-group mb-3 col-md-2">
                <input name="term" type="text" class="form-control" id="brand-search"
                    value="{{ request()->get('term', '') }}" placeholder="Enter Brand name">
            </div>

            <div class="form-group ml-3">
                <select class="form-control" id="developer" data-placeholder="Select Developer" name="devCheckboxs[]" multiple="multiple">
                    @foreach ($alldevs as $k => $_dev)
                        <option value="{{ $k }}" {{ in_array($k, request()->input('devCheckboxs', [])) ? 'selected' : '' }}>
                            {{ $_dev }}
                        </option>
                    @endforeach
                </select>
...
@section('scripts')
    ...
    <script type="text/javascript">
                    ...
                    if (data.code == 200) {
                        ...
                        $(".quickComments").each(function() {
                            ...
                            $.each(data.data, function(k, v) {
                                $(selecto).append("<option  value='" + k + "'>" + v +
                                    "</option>");
                            });
                            $(selecto).select2({
                                tags: true
        ...
        $(document).on("click", ".btn-create-task-multiple", function(e) {
            e.preventDefault();
            var $this = $(this).closest("form");
            $.ajax({
                url: $this.attr("action"),
