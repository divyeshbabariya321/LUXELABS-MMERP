...
class PushMagentoPagesInDB extends Command
{
    ...
    public function handle(): void
    {
        ...
        if (! empty($ids)) {
            ...
            $websites = StoreWebsite::whereIn('id', $ids)->where('api_token', '!=', '')->where('remote_software', '2')->where('website_source', 'magento')->get();
        } else {
            $websites = StoreWebsite::where('api_token', '!=', '')->where('remote_software', '2')->where('website_source', 'magento')->get();
        }
