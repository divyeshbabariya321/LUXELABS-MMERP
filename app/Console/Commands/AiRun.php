...
class AiRun extends Command
{
    ...
    public function handle(): void
    {
        try {
            ...
            $products = Product::where('stock', '>=', 1)->with('product_category')->leftJoin('log_scraper_vs_ai', 'products.id', '=', 'log_scraper_vs_ai.product_id')->where('ai_name', null)->orderByRaw($orderByPritority)->orderByDesc('products.listing_approved_at')->get(['products.id']);
            ...
            foreach ($products as $product) {
                ...
                foreach ($arrMedia as $media) {
                    ...
                    $arrImages[] = getMediaUrl($media); //'https://erp.amourint.com/' . $media->disk . '/' . $media->filename . '.' . $media->extension;
                }
