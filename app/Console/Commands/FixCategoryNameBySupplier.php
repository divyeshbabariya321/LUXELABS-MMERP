...
class FixCategoryNameBySupplier extends Command
{
    ...
    public function handle(): void
    {
        try {
            ...
            $this->category = Category::where('id', '>', 3)->whereNotNull('references')->where('references', '!=', '')->orderByDesc('id')->get();
            ...
            Product::where('is_scraped', 1)->where('category', '<', 4)->orderByDesc('id')->chunk(1000, function ($products) {
                echo 'Chunk again======================================================='."\n";
                $total = 1000;
        ...
        foreach ($scrapedProducts as $scrapedProduct) {
            ...
            foreach ($this->categoryRefrences as $categoryRefs) {
                ...
                if (! empty($categoryRefs['refrence'])) {
                    foreach ($categoryRefs['refrence'] as $refs) {
                        $cat = strtoupper($refs);
