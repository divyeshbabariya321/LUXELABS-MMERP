    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if ($this->guard()->attempt($credentials, $request->has('remember'))) {
            ...
            $user    = auth()->user();
            $user_ip = UserLoginIp::where('ip', $request->getClientIp())->where('user_id', $user->id)->orderByDesc('created_at')->first();
            ...
            if (! $user->isAdmin() && $user->is_auto_approval != 1) {
                ...
                $hubstaff_activities = HubstaffActivity::join('hubstaff_members', 'hubstaff_members.hubstaff_user_id', '=', 'hubstaff_activities.user_id')->whereDate('hubstaff_activities.starts_at', $date)->where('hubstaff_members.user_id', $user->id)->count();
                ...
                if ($hubstaff_activities) {
                    $activity = HubstaffActivitySummary::where('user_id', $user->id)->where('date', $date)->first();
                    if (! $activity) {
