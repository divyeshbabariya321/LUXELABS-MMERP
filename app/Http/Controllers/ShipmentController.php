...
use App\Customer;
use App\Email;
use App\Http\Requests\ShipmentSendEmailRequest;
...
class ShipmentController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $waybills = $waybills->orderByDesc('waybills.id')->select(
            [
                'waybills.*',
                'wi.due_date',
                'wi.invoice_currency',
     ...
     */
    public function sendEmail(ShipmentSendEmailRequest $request): RedirectResponse
    {
        $orders = Order::find($request->order_id);
        $fromEmail = $orders->storeWebsiteOrder->storeWebsite->magento_username;
     ...
     */
    public function viewSentEmail(Request $request): View
    {
        $emails = $this->emails->where('model_type', Order::class)
            ->where('model_id', $request->order_id)->orderByDesc('id')->get();
    ...
    public function generateShipment(Request $request): JsonResponse
    {
        $inputs = $request->all();
    ...
    public function viewWaybillTrackHistory(Request $request): View
    {
    ...
    public function saveBoxSize(Request $request): JsonResponse
    {
    ...
    public function editShipment(Request $request, $id): JsonResponse
    {
        $wayBill = Waybill::Find($id);
    ...
    public function saveShipment(Request $request, $id): JsonResponse
    {
        $wayBill = Waybill::Find($id);
        // if way bill found then start to insert data
        if ($wayBill) {
    ...
    {
        $wayBill = Waybill::leftJoin('waybill_invoices', 'waybill_invoices.shipment_number', 'waybills.awb')
    ...
    public function savePaymentInfo(Request $request): JsonResponse
    {
        try {
            $wayBill = Waybill::leftJoin('waybill_invoices', 'waybill_invoices.shipment_number', 'waybills.awb')
