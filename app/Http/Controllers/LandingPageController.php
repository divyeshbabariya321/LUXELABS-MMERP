...
class LandingPageController extends Controller
{
    ...
    {
        $title = 'Landing Page';
        $status = LandingPageProduct::STATUS;
        $store_websites = null;
        $statuses = LandingPageStatus::all()->pluck('name')->toArray();
    ...
    public function records(Request $request): JsonResponse
    {
        ...
        if ($brandName != null) {
            ...
            $records = LandingPageProduct::with(['landing_page_status'])->join('products as p', function ($query) use ($brandId) {
                $query->on('p.id', '=', 'landing_page_products.product_id');
                $query->where('p.brand', $brandId);
            });
        } else {
            $records = LandingPageProduct::with(['landing_page_status'])->join('products as p', 'p.id', 'landing_page_products.product_id');
        }
        ...
        foreach ($records->items() as &$rec) {
            ...
            $previousVal = isset(LandingPageProduct::STATUS[$rec->status]) ? LandingPageProduct::STATUS[$rec->status] : $rec->status;
    ...
    public function pushToShopify(Request $request, $id): JsonResponse
    {
        $landingPage = LandingPageProduct::where('id', $id)->first();
    ...
    public function pushToMagentoPro(Request $request, $id): JsonResponse
    {
        $landingPage = LandingPageProduct::where('id', $id)->first();
        $queueName = [
            '1' => 'mageone',
    ...
    public function updateMagentoStock(Request $request, $id): JsonResponse
    {
        $landingPage = LandingPageProduct::where('id', $id)->first();
        // if stock status exist then store it
        if ($request->stock_status != null) {
    ...
    public function updateTime(Request $request): RedirectResponse
    {
    ...
    public function deleteImage($id, $productId): JsonResponse
    {

        Mediables::where('mediable_type', Product::class)
    ...
    public function changeStore(Request $request, $id): JsonResponse
    {
        $landing = LandingPageProduct::find($id);
