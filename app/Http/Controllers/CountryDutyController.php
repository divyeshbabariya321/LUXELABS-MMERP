...
class CountryDutyController extends Controller
{
    ...
    public function saveCountryGroup(Request $request): JsonResponse
    {
        // validation report generator
        $validator = Validator::make($request->all(), [
            'name' => 'required',
    ...
    {
        $records = CountryDuty::leftJoin('duty_groups as dg', 'dg.id', 'country_duties.duty_group_id');
    ...
    {
        $modal = CountryDuty::where('id', $id)->first();
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $data = CountryDuty::create($data);

        return response()->json(['code' => 200, 'data' => $data]);
    }

    public function updateGroupField(Request $request): JsonResponse
    {
        $params = $request->all();

        if (isset($params['id'])) {
            $countryGroup = DutyGroup::find($params['id']);
            if ($countryGroup) {
