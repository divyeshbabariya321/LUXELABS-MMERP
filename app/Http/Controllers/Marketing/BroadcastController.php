...
class BroadcastController extends Controller
{
     ...
     *
     * @param Customer  id , term , date , number , broadcast , manual , remark , name
    ...
    public function index(Request $request)
    {
        if ($request->term || $request->total || $request->date || $request->number || $request->broadcast || $request->manual || $request->remark || $request->name || $request->customrange || $request->dnd || $request->whats_number || $request->lastBroadcast || $request->notDelivered || $request->broadcastSend || $request->manualApproval) {
            ...
            if (request('total') != null) {
                ...
                if (request('total') == 1 && request('customrange') != null) {
                    ...
                    $query->whereHas('customerMarketingPlatformActive', function ($qu) {
                        $qu->where('active', 1);
                    })->where('do_not_disturb', 0);
                }
        ...
        if ($request->phone_term || $request->phone_date || $request->phone_customrange) {
            ...
            //global search term
            if (request('phone_term')) {
                $query->where('number', 'LIKE', "%{$request->phone_term}%");
            }

            if (request('phone_date')) {
                $date = request('phone_date');
            } else {
                $date = '';
            }

            if (request('phone_customrange')) {
            ...
            $numbers = WhatsappConfig::select('number')->get();
            $date = '';
            $startDate = '';
            $endDate = '';
        }
    ...
    public function addManual(Request $request): JsonResponse
    {
        ...
        if ($customer != null && $request->type == 1) {
            ...
            $whatsappConfigs = WhatsappConfig::where(['is_customer_support' => 0, 'status' => 1])->get();
     ...
     * @param  \App\Customer  $request  ->id
     */
    public function updateWhatsAppNumber(Request $request): JsonResponse
    {
    ...
    public function broadCastSendMessage(Request $request): JsonResponse
    {
    ...
    public function getCustomerBroadcastList(Request $request): JsonResponse
    {
        $id = $request->id;
        $customer = Customer::findOrFail($id);
    ...
    public function saveGlobalValues(Request $request): RedirectResponse
    {
        $numbers = WhatsappConfig::select('is_customer_support', 'frequency', 'send_start', 'send_end')->where('is_customer_support', 0)->get();
    ...
    public function getCustomerCountEnable(): JsonResponse
    {
    ...
    public function switchBroadcast(Request $request): RedirectResponse
    {
        $id = $request->id;
        if ($id == $request->newId) {
        ...
        $messages = ImQueue::select('number_from')->where('number_from', $whatsAppOld->number)->get();
        foreach ($messages as $message) {
