...
class HsCodeController extends Controller
{
    ...
    public function mostCommon(Request $request)
    {
        ...
        if ($request->category || $request->combination) {
            ...
            $productss = $query->orderByDesc('total')->take(100)->get();
        } else {
            $productss = $query->select('*', DB::raw('count(*) as total'))
                ->where('category', '>', 3)->where('stock', 1)->groupBy('category')->groupBy('composition')->whereNotNull('composition')->where('composition', '!=', '')->orderByDesc('total')->get();
        }
    ...
    {
        if ($request->category != null) {
            $categories = Category::select('id', 'title')->where('id', $request->category)->orderBy('title')->get()->toArray();
        } else {
            $categories = Category::orderBy('title')->whereNotIn('id', [1, 2, 3])->get();
        }
