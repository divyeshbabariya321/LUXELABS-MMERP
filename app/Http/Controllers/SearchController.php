...
class SearchController extends Controller
{
    public function search(Stage $stage, Request $request)
    {
        ...
        if ($export) {
            return \Excel::download(new ProductExport($products->get()), 'export.xlsx');
        }
        ...
        if ($request->ajax()) {
            $html = view('partials.image-load', ['products' => $data['products'], 'data' => $data, 'selected_products' => ($request->selected_products ? json_decode($request->selected_products) : []), 'model_type' => $model_type, 'categories' => $categories])->render();
        ...
        $data['pending_products_count'] = Product::getPendingProductsCount($roletype);
        $data['brands'] = Brand::getAll();
        $data['colors'] = (new Colors)->all();
        $data['stage'] = $stage;
        $data['suppliers'] = Supplier::getProductSuppliers();

        return view('partials.grid', $data);
    }

    public function getPendingProducts($roletype): View
    {
        $stage = new Stage;
