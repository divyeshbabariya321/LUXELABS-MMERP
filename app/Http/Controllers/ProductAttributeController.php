...
class ProductAttributeController extends Controller
{
    ...
    public function edit(Sizes $sizes, Product $productattribute): View
    {
        $data = [];
    ...
    public function delSizeQty($id): RedirectResponse
    {
    ...
    public function update(Request $request, Guard $auth, Product $productattribute, Stage $stage): RedirectResponse
    {
        ...
        } elseif ($productattribute->stage == $stage->get('Attribute')) {
        ...
        foreach ($images as $image) {
            ...
            try {
                $result = $proxy->catalogProductAttributeMediaCreate(
                    $sessionId,
                    $reference_final_sku,
                    ['file' => $file, 'label' => pathinfo($image->getBasenameAttribute(), PATHINFO_FILENAME), 'position' => ++$i, 'types' => $types, 'exclude' => 0]
                );
            } catch (Exception $e) {
                $errors++;
