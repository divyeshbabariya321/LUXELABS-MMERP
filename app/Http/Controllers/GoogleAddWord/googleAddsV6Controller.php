...
class googleAddsV6Controller extends Controller
{
    ...
    public function main(Request $request)
    {
        ...
        $gClient->setApplicationName(config('settings.google_ads_client_application_name'));
        $gClient->setClientId(config('settings.google_ads_client_id'));
        $gClient->setClientSecret(config('settings.google_ads_client_secret'));
        $gClient->setDeveloperKey(config('settings.google_ads_developer_key'));
        ...
        try {
            if ($request->location) {
                return self::runExample($googleAdsClient, self::CUSTOMER_ID, [$request->location], $request->language ?? self::LANGUAGE_ID, [$request->keyword], self::PAGE_URL);
            } else {
                return self::runExample($googleAdsClient, self::CUSTOMER_ID, [], $request->language ?? self::LANGUAGE_ID, [$request->keyword], self::PAGE_URL);
            }
