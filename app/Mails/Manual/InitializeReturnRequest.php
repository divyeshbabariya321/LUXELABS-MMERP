...
class InitializeReturnRequest extends Mailable
{
    ...
    public function build(): static
    {
        ...
        if ($customer) {
            if ($customer->store_website_id > 0) {
                ...
                $template = MailinglistTemplate::getIntializeReturn($customer->store_website_id);
            } else {
                $emailAddress = EmailAddress::where('store_website_id', self::STORE_ERP_WEBSITE)->first();
                if ($emailAddress) {
                    $this->fromMailer = $emailAddress->from_address;
                }
                $template = MailinglistTemplate::getIntializeReturn();
            }
            if ($template) {
